...
- **Klassische Puzzleformen:** `pieceShape=puzzle` erzeugt rasterabhängig passende Zapfen und Einbuchtungen mit geradem Außenrand. Records tragen `puzzleEdges` (oben/rechts/unten/links), die der sichtbare ForEach-Flow ins Sprite-Template überträgt. `PuzzleShape.ts` berechnet gemeinsame Kanten und Bézierkonturen; `PuzzlePieceRenderer.ts` zeichnet in Vorschau und Runtime dieselbe SVG-Form inklusive Bildinhalt der Zapfen. Nur die Teileform ist im Run anklickbar. Die Rasterzelle bleibt der Einrastanker; `trayLayout` bietet Ablageplätze mit Zapfenabstand in unveränderter Zielgröße; der Ablagebereich darf über die Splitterbox hinauswachsen. PuzzleNeu reserviert dafür eine Haupt-Stage mit 60×40 Zellen, Zielplattform rechts bei x=35 und Bedienknöpfen unterhalb der Ablage. Rechteckmodus bleibt Standard, Eindeutigkeit der Formen ist nicht vorgeschrieben.
