...
/** Die KI lädt diese Ressourcen per URL; Vite kopiert docs/ sonst nicht in dist/. */
function knowledgeAssetsPlugin(): Plugin {
    return {
        ...
        generateBundle() {
            ...
            const walk = (dir: string) => {
                for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
                    ...
                    if (entry.isDirectory()) {
                        walk(full);
                    } else if (/\.(md|json)$/.test(entry.name) && entry.name !== 'QA_Report.md') {
                        this.emitFile({
                            type: 'asset',
                            fileName: `docs/${path.relative(root, full).split(path.sep).join('/')}`,
...
export default defineConfig({
    ...
    plugins: [mediaManifestPlugin(), knowledgeAssetsPlugin()],
    server: {
        port: 5173,
        ...
        proxy: {
            ...
            '/kb/kb.json': 'http://localhost:8080'
        }
    },
    build: {
        outDir: 'dist',
