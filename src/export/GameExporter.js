...
export class GameExporter {
     ...
     */
    async exportHTMLCompressed(project) {
     ...
     */
    async exportJSONCompressed(project) {
        const cleanedProject = this.getCleanProject(project);
        // Embed images as Base64
        try {
            await this.embedImages(cleanedProject);
    ...
    getCleanProject(project) {
        ...
        if (!clean.meta)
            clean.meta = {};
        const mainStage = clean.stages?.find((s) => s.type === 'main');
        if (mainStage) {
            if (mainStage.gameName)
                clean.meta.name = mainStage.gameName;
            if (mainStage.author)
                clean.meta.author = mainStage.author;
            if (mainStage.description)
                clean.meta.description = mainStage.description;
        }
     ...
     */
    generateStandaloneHTML(project, runtimeJS) {
...
<body>
     ...
     */
    generateCompressedHTML(project, runtimeJS) {
...
<body>
     ...
     */
    async embedImages(project) {
        const imageRefs = new Map(); // path -> dataUrl
