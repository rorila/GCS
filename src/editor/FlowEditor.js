...
export class FlowEditor {
     ...
     * Berücksichtigt nun auch stage-spezifische Flow-Diagramme.
     */
    syncAllTasksFromFlow(project) {
        console.log(`[FlowEditor] Starting global sync for tasks in project: ${project.meta.name}`);
        const syncFromCharts = (charts) => {
            if (!charts)
                return;
            project.tasks.forEach(task => {
                ...
                if (flowData && flowData.elements && flowData.elements.length > 0) {
                    ...
                    this.stateManager.setContext(task.name);
                    try {
        ...
        // 1. Sync from global project charts
        syncFromCharts(project.flowCharts);
        // 2. Sync from all stage-specific charts
        if (project.stages) {
    ...
    getTaskDefinitionByName(taskName) {
        if (!this.project)
            return null;
    ...
    getAllVariables() {
        ...
        let all = [...(this.project.variables || [])];
        if (this.project.stages) {
            this.project.stages.forEach(s => {
                if (s.variables) {
                    all = [...all, ...s.variables];
    ...
    constructor(containerId, editor) {
        ...
        this.currentSelectedStageObjectId = null;
        this.suggestedTaskName = null; // Für automatische Namensübernahme bei Drop
        // Navigation History
        this.contextHistory = [];
    ...
    setProject(project) {
        ...
        this.flowStage = new TFlowStage('FlowStage', this.project.flow.stage?.cols ?? 100, this.project.flow.stage?.rows ?? 100, this.project.flow.stage?.cellSize ?? 20);
    ...
    getTargetFlowCharts(taskName) {
        ...
        if (!activeStage.flowCharts)
            activeStage.flowCharts = {};
        return activeStage.flowCharts;
    }
    deleteCurrentTaskFlow() {
    ...
    updateFlowSelector() {
        ...
        viewGroup.appendChild(overviewOpt);
        this.flowSelect.appendChild(viewGroup);
        ...
        if (!activeStage || activeStage.type === 'main') {
            ...
            const globalTasksFound = new Set();
    ...
    switchActionFlow(context, addToHistory = true) {
        if (this.currentFlowContext === context)
            return;
    ...
    findActionInSequence(sequence, name) {
        if (!sequence)
            return null;
    ...
    updateGlobalActionDefinition(actionData) {
        if (!this.project)
            return;
        ...
        const idx = targetCollection.findIndex((a) => a.name === name);
        if (idx !== -1) {
     ...
     */
    generateUniqueActionName(baseName = 'Action') {
        let counter = 1;
        ...
        if (this.project?.stages) {
            this.project.stages.forEach(s => {
                if (s.actions)
                    s.actions.forEach(a => existingNames.add(a.name));
            });
        }
    ...
    generateUniqueVariableName(baseName = 'neueVariabel') {
        let counter = 1;
        ...
        if (this.project?.stages) {
            this.project.stages.forEach(s => {
                if (s.variables)
                    s.variables.forEach(v => existingNames.add(v.name));
            });
        }
        this.nodes.forEach(n => {
    ...
    generateUniqueTaskName(baseName = 'Task') {
        let counter = 1;
        let finalName = baseName;
        const existingNames = new Set();
        if (this.project?.tasks) {
        ...
        if (this.project?.stages) {
            this.project.stages.forEach(s => {
                if (s.tasks)
                    s.tasks.forEach(t => existingNames.add(t.name));
            });
        }
        this.nodes.forEach(n => {
            if (n.getType() === 'Task') {
     ...
     */
    ensureTaskExists(taskName, description) {
        if (!this.project)
            return;
    ...
    syncToProject() {
        ...
        proxyNodes.forEach(proxy => {
            ...
            if (ghosts.length > 0) {
                if (!proxy.data.ghostPositions)
                    proxy.data.ghostPositions = {};
                ghosts.forEach(g => {
        ...
        if (this.currentFlowContext === 'global') {
            ...
            // Also keep legacy 'flow' for backwards compatibility if saving to project root
            if (targetCharts === this.project.flowCharts) {
                if (!this.project.flow) {
                    this.project.flow = {
                ...
                this.project.flow.stage = targetCharts.global.stage;
            }
            ...
            if (task) {
                ...
                targetCharts[this.currentFlowContext] = { elements, connections };
                ...
                // VARIABLE SYNC: Sync variables from nodes
                this.syncVariablesFromFlow();
            }
        }
        // Notify Editor to trigger auto-save
     ...
     */
    syncTaskParameters(task, elements) {
        // Extract all ${param} references from Action nodes
        const usedParams = new Map();
        ...
        elements.forEach((el) => {
            ...
            while ((match = paramRegex.exec(jsonStr)) !== null) {
                ...
                if (!usedParams.has(paramName)) {
                    usedParams.set(paramName, { types: new Set(), sources: [] });
                }
        ...
        const existingParamNames = new Set(task.params.map((p) => p.name));
        ...
        const removedParams = [];
        task.params = task.params.filter((p) => {
    ...
    syncTaskFromFlow(task, elements, connections) {
        ...
        const buildSequence = (nodeId, targetSeq, stopSet = new Set()) => {
            if (!nodeId || visited.has(nodeId) || stopSet.has(nodeId))
                return;
            ...
            if (nodeType === 'Action' || nodeType === 'Task') {
                ...
                const bodyConn = connections.find(c => c.startTargetId === node.id &&
                    (c.data?.anchorType === 'true' || c.data?.startAnchorType === 'true' || c.data?.branchType === 'true'));
                if (bodyConn) {
                    ...
                    const buildSubSequence = (sNodeId, sTargetSeq, sStopNodeId, sVisited) => {
                        if (!sNodeId || sNodeId === sStopNodeId || sVisited.has(sNodeId))
                            return;
                        ...
                        if (sType === 'Action' || sType === 'Task') {
                            ...
                            if (out)
                                buildSubSequence(out.endTargetId, sTargetSeq, sStopNodeId, sVisited);
                        }
                ...
                const exitConn = connections.find(c => c.startTargetId === node.id &&
                    (c.data?.anchorType === 'false' || c.data?.startAnchorType === 'false' || c.data?.branchType === 'false'));
                if (exitConn) {
    ...
    loadFromProject() {
        ...
        if (this.currentFlowContext === 'global') {
            ...
            const projectFlowIdx = (this.project.flowCharts?.global) || this.project.flow;
            sourceData = stageFlowIdx || projectFlowIdx;
            if (sourceData)
                console.log(`[FlowEditor] SUCCESS: Loaded GLOBAL flow from ${stageFlowIdx ? 'STAGE' : 'PROJECT'}.`);
        }
        else {
        ...
        if (sourceData.elements) {
            sourceData.elements.forEach((data) => {
        ...
        if (sourceData.connections) {
            sourceData.connections.forEach((data) => {
        ...
        if (node) {
            ...
            if (data.properties) {
                if (data.properties.name)
                    node.Name = data.properties.name;
                if (data.properties.details)
                    node.Details = data.properties.details;
                if (data.properties.description)
                    node.Description = data.properties.description;
                if (data.properties.text && !data.properties.name)
                    node.Name = data.properties.text;
            }
     ...
     */
    refreshEmbeddedTask(proxyNode) {
        if (!this.project || !proxyNode.data?.isExpanded || !proxyNode.data?.sourceTaskName)
            return;
        ...
        if (existingGhostNodes.length > 0) {
            if (!proxyNode.data.ghostPositions)
                proxyNode.data.ghostPositions = {};
        ...
        toDeleteConns.forEach(c => {
            ...
            if (el.parentNode === this.canvas)
                this.canvas.removeChild(el);
            if (sh.parentNode === this.canvas)
                this.canvas.removeChild(sh);
            if (eh.parentNode === this.canvas)
                this.canvas.removeChild(eh);
     ...
     */
    generateFlowFromActionSequence(task) {
        ...
        let nextNodeId = 0;
        const getNewId = (type) => `auto_${type}_${nextNodeId++}_${Date.now()}`;
        // Recursive processor
        const process = (sequence, startNodeId, startAnchor = 'output', branch, startX = 400, startY = 200) => {
    ...
    createNode(type, x, y, initialName) {
        let node;
        ...
        switch (baseType) {
            ...
            case 'VariableDecl':
                ...
                if (kind === 'threshold')
                    node.data.variable.threshold = 0;
                if (kind === 'trigger')
                    node.data.variable.triggerValue = '';
                if (kind === 'timer')
                    node.data.variable.duration = 5000;
                if (kind === 'range') {
                    node.data.variable.min = 0;
                    node.data.variable.max = 100;
                }
                if (kind === 'list') {
                    node.data.variable.type = 'list';
                    node.data.variable.initialValue = '[]';
                }
                if (kind === 'random') {
                    node.data.variable.min = 0;
                    node.data.variable.max = 100;
                    node.data.variable.isRandom = true;
                }
                node.updateVisuals?.();
                break;
            case 'While':
            case 'For':
            case 'Repeat':
                node = new FlowLoop(id, x, y, this.canvas, this.flowStage.cellSize, type);
                node.Name = type;
                node.updateVisuals?.();
                break;
            case 'Start':
    ...
    deleteElementFromProject(type, name, index) {
        if (!this.project)
            return;
        if (type === 'Action') {
    ...
    setupNodeListeners(node) {
        ...
        const setupAnchor = (anchor, isOutput, branchType) => {
            if (!anchor)
                return;
            anchor.addEventListener('mousedown', (e) => {
                ...
                e.preventDefault();
                if (!isOutput)
                    return; // Currently we only drag from Output to Input
                const rect = this.canvas.getBoundingClientRect();
                const x = e.clientX - rect.left;
                const y = e.clientY - rect.top;
    ...
    importTaskGraph(targetNode, task, isLinked = false) {
        ...
        const projectChart = this.project?.flowCharts?.[task.name];
        ...
        flowChart.elements.forEach((data) => {
            ...
            if (newNode) {
                ...
                if (newNode.getType() === 'Start')
                    importedStart = newNode;
            }
        });
        ...
        if (flowChart.connections) {
            flowChart.connections.forEach((data) => {
                ...
                if (startId && endId) {
                    ...
                    if (startNode && endNode) {
                        ...
                        const savedConn = isLinked && targetNode.data?.ghostConnections?.find((gc) => gc.startOriginalId === origStartId &&
        ...
        if (isLinked && importedStart) {
            ...
            const savedConn = targetNode.data?.ghostConnections?.find((gc) => gc.startOriginalId === origStartId &&
                gc.endOriginalId === origEndId &&
                gc.originalStartAnchorType === origStartAnchor);
            if (savedConn) {
                conn.data = {
        ...
        if (isLinked) {
            ...
            if (task.description)
                targetNode.Description = task.description;
        }
        else {
    ...
    handleNodeContextMenu(e, node) {
        ...
        if (node instanceof FlowTask) {
            ...
            const flowChart = this.project?.flowCharts?.[node.Name] || taskDef?.flowChart;
        ...
        if (node.getType() === 'Task' && this.project) {
            ...
            const linkItems = this.project.tasks.map(t => ({
                label: t.name,
            ...
            // Option B: Import
            const importItems = this.project.tasks.map(t => ({
                label: t.name,
            ...
            if (libraryTasks.length > 0) {
                const libraryItems = libraryTasks.map(t => ({
                    label: `📋 ${t.name}`,
            ...
            // Option A: Link
            const linkItems = this.project.actions.map(a => ({
                label: a.name,
            ...
            // Option B: Copy
            const copyItems = this.project.actions.map(a => ({
                label: a.name,
     ...
     */
    copyLibraryTaskAsTemplate(node, libraryTask) {
        if (!this.project)
            return;
        ...
        if (taskCopy.params) {
            taskCopy.params.forEach((p) => p.fromLibrary = true);
        }
        ...
        if (libraryTask.flowChart) {
            ...
            if (flowChartCopy.elements) {
                flowChartCopy.elements.forEach((el) => {
                    const oldId = el.id;
            ...
            if (flowChartCopy.connections) {
                flowChartCopy.connections.forEach((conn) => {
                    if (conn.startTargetId && idMapping[conn.startTargetId])
                        conn.startTargetId = idMapping[conn.startTargetId];
                    if (conn.endTargetId && idMapping[conn.endTargetId])
                        conn.endTargetId = idMapping[conn.endTargetId];
                });
            }
            // Automatisch einen Task-Knoten am Anfang einfügen (falls nicht vorhanden)
            const hasTaskNode = flowChartCopy.elements?.some((el) => el.type === 'Task');
            if (!hasTaskNode && flowChartCopy.elements?.length > 0) {
                ...
                flowChartCopy.elements.forEach((el, idx) => {
        ...
        if (this.currentFlowContext === 'event-map' && node.data.stageObjectId && node.data.eventName) {
            ...
            if (obj) {
                if (!obj.Tasks)
                    obj.Tasks = {};
                obj.Tasks[node.data.eventName] = newName;
                console.log(`[FlowEditor] CLONE: Updated mapping for ${obj.name}.${node.data.eventName} -> ${newName}`);
            }
        }
        if (libraryTask.description)
            node.Description = libraryTask.description;
        node.setDetailed(true);
    ...
    registerActionsFromTask(task) {
        ...
        const processSequence = (sequence) => {
            ...
            sequence.forEach(item => {
                ...
                if (item.body)
                    processSequence(item.body);
                if (item.then)
                    processSequence(item.then);
                if (item.else)
                    processSequence(item.else);
            });
        };
        ...
        if (task.flowChart && task.flowChart.elements) {
            task.flowChart.elements.forEach((el) => {
                if (el.type === 'Action') {
    ...
    openActionEditor(node) {
        ...
        if (!dialogData.type)
            dialogData.type = nodeData.type || 'property';
        if (!dialogData.target)
            dialogData.target = nodeData.target || '';
        if (!dialogData.changes)
            dialogData.changes = nodeData.changes || {};
     ...
     */
    renameActionInMemory(oldName, newName) {
        this.nodes.forEach(n => {
            if (n.getType() === 'Action') {
                if (n.Name === oldName)
                    n.setText(newName);
                if (n.data) {
                    if (n.data.name === oldName)
                        n.data.name = newName;
                    if (n.data.actionName === oldName)
                        n.data.actionName = newName;
                }
            }
            else if (n.getType() === 'Condition') {
                if (n.data) {
                    if (n.data.thenAction === oldName)
                        n.data.thenAction = newName;
                    if (n.data.elseAction === oldName)
                        n.data.elseAction = newName;
                }
            }
        });
    }
    ...
    handleGlobalMove(e) {
        if (!this.isDraggingHandle || !this.activeConnection || !this.activeHandle)
            return;
        const rect = this.canvas.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const y = e.clientY - rect.top;
     ...
     */
    findClosestAnchor(x, y, radius, filter) {
        let closest = null;
    ...
    get name() { return 'Flow Grid Settings'; }
    get GridColumns() { return this.flowStage.cols; }
    set GridColumns(v) {
        this.flowStage.cols = v;
        this.updateGrid();
        this.syncToProject();
    }
    get GridRows() { return this.flowStage.rows; }
    set GridRows(v) {
        this.flowStage.rows = v;
        this.updateGrid();
        this.syncToProject();
    }
    get CellSize() { return this.flowStage.cellSize; }
    set CellSize(v) {
        this.flowStage.cellSize = v;
        this.updateGrid();
        this.syncToProject();
    }
    get SnapToGrid() { return this.flowStage.snapToGrid; }
    set SnapToGrid(v) {
        this.flowStage.snapToGrid = v;
        this.updateGrid();
        this.syncToProject();
    }
    get ShowGrid() { return this.flowStage.showGrid; }
    set ShowGrid(v) {
        this.flowStage.showGrid = v;
        this.updateGrid();
        this.syncToProject();
    }
    get BackgroundColor() { return this.flowStage.style.backgroundColor || '#1e1e1e'; }
    ...
    generateEventMap() {
        ...
        const objectsWithTasks = this.getCurrentObjects().filter(obj => {
            ...
            checkBindings(obj);
            if (!hasTasks && !hasBindings)
                return false;
            if (this.filterText) {
                const nameMatches = obj.name.toLowerCase().includes(this.filterText);
                const taskNames = hasTasks ? Object.values(tasks) : [];
        ...
        const variablesWithTasks = this.getAllVariables().filter(v => {
            ...
            const topLevelEvents = Object.keys(v).filter(k => k.startsWith('on') && v[k]);
            const taskEvents = v.Tasks ? Object.keys(v.Tasks).filter(k => k.startsWith('on') && v.Tasks[k]) : [];
            const hasTasks = topLevelEvents.length > 0 || taskEvents.length > 0;
            if (!hasTasks)
                return false;
            if (this.filterText) {
                const nameMatches = v.name.toLowerCase().includes(this.filterText);
                const taskMatches = [...topLevelEvents.map(e => v[e]), ...taskEvents.map(e => v.Tasks[e])]
        ...
        objectsWithTasks.forEach(obj => {
            ...
            objNode.autoSize();
            objNode.Details = obj.className || 'Object';
            // Extract reactive bindings from object to show them in the diagram
            const bindings = {};
            const gatherBindings = (target, prefix = '') => {
                if (!target)
                    return;
                Object.entries(target).forEach(([key, val]) => {
                    if (typeof val === 'string' && val.startsWith('${')) {
        ...
        variablesWithTasks.forEach(variable => {
            ...
            const topLevelEvents = Object.entries(variable)
                .filter(([k, v]) => k.startsWith('on') && typeof v === 'string' && v);
            const taskEvents = variable.Tasks ? Object.entries(variable.Tasks)
                .filter(([k, v]) => k.startsWith('on') && typeof v === 'string' && v) : [];
    ...
    createMapTaskNode(taskName, eventName, x, y, sourceId) {
        ...
        taskNode.autoSize();
        const taskDef = this.project.tasks.find((t) => t.name === taskName);
        if (taskDef && taskDef.description) {
            taskNode.Description = taskDef.description;
        }
        // Check if this task uses a library task internally
        let usedLibraryTaskName = null;
        const flowData = taskDef?.flowChart || taskDef?.flowGraph ||
    ...
    createEventLink(sourceNode, targetNode, sourceName, eventName) {
        const conn = new FlowConnection(this.canvas, 0, 0, 0, 0);
        conn.setGridConfig(this.flowStage.cellSize);
    ...
    generateElementOverview() {
        ...
        currentVariables.forEach((v) => {
            ...
            if (width > maxVarWidth)
                maxVarWidth = width;
        });
        const varX = taskX + maxTaskWidth + 80;
        // 2. Collect Usage Data
        const stageRelevantTasks = new Set();
        const usedActions = new Set();
        const activeStageId = activeStage ? activeStage.id : null;
        const isGlobalView = !activeStage || activeStage.type === 'main';
        console.log(`[FlowEditor] generateElementOverview - activeStage: ${activeStage?.name} (${activeStageId}), isGlobalView: ${isGlobalView}`);
        // A. Tasks used by objects of the current stage
        this.getCurrentObjects().forEach(obj => {
            const mappings = obj.Tasks || {};
            Object.values(mappings).forEach(t => {
                if (typeof t === 'string')
                    stageRelevantTasks.add(t);
            });
        });
        ...
        relevantVars.forEach(v => {
            ...
            // Check Tasks object
            const taskMappings = v.Tasks || {};
            Object.values(taskMappings).forEach(t => {
                if (typeof t === 'string' && t)
                    stageRelevantTasks.add(t);
            });
        });
        // B. Tasks defined in the stage's local flowCharts
        if (activeStage?.flowCharts) {
            Object.keys(activeStage.flowCharts).forEach(taskName => {
                if (taskName !== 'global')
                    stageRelevantTasks.add(taskName);
            });
        }
            ...
            if (stageRelevantTasks.has(t.name) && t.actionSequence) {
                const processSeq = (seq) => {
                    seq.forEach(item => {
                        ...
                        if (item.type === 'condition') {
                            if (item.thenAction)
                                usedActions.add(item.thenAction);
                            if (item.elseAction)
                                usedActions.add(item.elseAction);
                        }
                    });
                };
        ...
        // Also check task-specific flowCharts for actions (even if not synced to actionSequence yet)
        const relevantCharts = {};
        if (activeStage?.flowCharts) {
            Object.entries(activeStage.flowCharts).forEach(([name, chart]) => {
                if (name !== 'global')
                    relevantCharts[name] = chart;
            });
        }
        Object.values(relevantCharts).forEach((chart) => {
            (chart.elements || []).forEach((el) => {
                const name = el.data?.name || el.data?.actionName || el.properties?.name;
                if (el.type === 'Action' && name)
                    usedActions.add(name);
                if (el.type === 'Condition' && el.data) {
                    if (el.data.thenAction)
                        usedActions.add(el.data.thenAction);
                    if (el.data.elseAction)
                        usedActions.add(el.data.elseAction);
                }
            });
        });
        console.log(`[FlowEditor] usedActions for this stage:`, Array.from(usedActions));
        // Map ObjectName -> StageId for all objects (Context-Aware Logic)
        const objectStageMap = new Map();
        if (this.project.stages) {
            this.project.stages.forEach(stage => {
                if (stage.objects) {
        ...
        // List of global services and system singletons
        ...
        const actionItems = currentActions
            .map((action) => {
            const isLocal = localActions.some((la) => la.name === action.name);
            ...
            if (isGlobalView)
                return true;
            // 1. Service / Singleton Check (High Priority)
            ...
            if (prefix && (globalServices.has(prefix) || globalSingletons.has(prefix))) {
                // Special Rule: Hide redundant navigation for StageController
                if (prefix === 'StageController') {
                    const method = action.method || action.methodName;
                    if (method === 'goToStage') {
                        const targetStageId = action.params && action.params[0];
                        if (targetStageId === activeStageId) {
                            console.log(`[FlowEditor] HIDING Redundant Navigation: '${actionName}' targets current stage '${activeStageId}'`);
                            return false;
                        }
                    }
                    if (method === 'goToMainStage' && activeStage?.type === 'main') {
                        console.log(`[FlowEditor] HIDING Redundant Navigation: '${actionName}' targets main while already on main`);
                        return false;
                    }
                }
                return true; // Always show system services
            }
            // 2. Used Actions Check (Explicit contextual usage)
            if (usedActions.has(actionName)) {
                return true;
            }
            // 3. Explicit Object Stage Check
            const targetsToCheck = [prefix, action.target, action.targetName, action.source].filter(Boolean);
            for (const t of targetsToCheck) {
                if (objectStageMap.has(t)) {
                    const targetStageId = objectStageMap.get(t);
                    if (targetStageId !== activeStageId) {
                        console.log(`[FlowEditor] REJECTED Action '${actionName}': Target '${t}' resides in stage '${targetStageId}'`);
                        return false;
                    }
                    // If it matches an object in the CURRENT stage, we could return true here, 
                    // but let's continue to other checks (for heuristics) just in case.
                }
            }
            ...
            // 5. Final Fallback for Unused/Ambiguous Actions
            // If it's a standalone action (no dot, no target) and not used here, hide it in stage view.
            if (dotIndex === -1 && targetsToCheck.length === 0) {
                console.log(`[FlowEditor] HIDING Action '${actionName}': Not used in stage and no clear target.`);
                return false;
            }
            // If it has a dot or targets but they are unknown (likely from another stage's custom logic), hide it.
            if (targetsToCheck.length > 0 || dotIndex !== -1) {
                console.log(`[FlowEditor] HIDING Action '${actionName}': Unknown targets/prefix in current stage context.`);
                return false;
            }
            return true;
        });
        console.log(`[FlowEditor] Final visible actions: ${actionItems.length}`);
        ...
        const taskItems = currentTasks
            .map((task) => {
            const isLocal = localTasks.some((lt) => lt.name === task.name);
            return { task, isLocal };
        })
            .filter((item) => {
            ...
            if (stageRelevantTasks.has(taskName))
                return true;
            return false;
        });
        taskItems.sort((a, b) => (a.task.name || "").localeCompare(b.task.name || ""));
        taskItems.forEach((item, displayIdx) => {
            const task = item.task;
     ...
     */
    highlightUnusedActions(highlight) {
        const unusedDetails = [];
        ...
        this.nodes.forEach(node => {
            ...
            const name = node.Name || node.taskName || "unknown";
            if ((nodeType === 'Action' || nodeType === 'Task' || nodeType === 'VariableDecl' || nodeType === 'TVariable') && isUnused) {
                ...
                if (nodeType === 'Task')
                    unusedTaskCount++;
                if (nodeType === 'VariableDecl' || nodeType === 'TVariable')
                    unusedVariableCount++;
                if (highlight) {
    ...
    updateScrollArea() {
        ...
        let maxY = 2000;
        this.nodes.forEach(n => {
            const bounds = { x: n.x + 400, y: n.y + 400 };
            if (bounds.x > maxX)
                maxX = bounds.x;
            if (bounds.y > maxY)
                maxY = bounds.y;
