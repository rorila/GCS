...
interface PaletteCategory {
    name: string;
    items: PaletteItem[];
    action?: string; // Optional action to execute on category click
}

interface PaletteConfig {
    meta: { name: string; version: string; layout: string };
    toolbar?: ToolbarButton[]; // Optional toolbar
...
export class JSONComponentPalette {
    ...
    constructor(toolbarContainerId: string, paletteContainerId: string) {
        ...
        if (!toolbar) throw new Error(`Container ${toolbarContainerId} not found`);
        if (!palette) throw new Error(`Container ${paletteContainerId} not found`);
     ...
     */
    async loadFromJSON(json: PaletteConfig): Promise<void> {
        this.config = json;
