...
export class JSONInspector {
    constructor(containerId) {
        ...
        this.layoutConfig = null; // Custom inspector layout configuration
    ...
    setupBindings() {
        ...
        const processObject = (obj) => {
            ...
            if (obj._rowItems && Array.isArray(obj._rowItems)) {
                obj._rowItems.forEach((item) => processObject(item));
            }
        };
    ...
    async update(object) {
        ...
        if (object && object.className === 'TInspectorTemplate') {
            ...
            this.renderInspectorDesigner(object);
            return;
        }
        ...
        if (isProject) {
            ...
            let dynamicObjects = [];
            const hasDynamicProps = typeof object.getInspectorProperties === 'function';
            ...
            if (object.variableType || object.value !== undefined) {
                ...
                object._supportedEvents = variableEvents.map(evt => ({ key: evt }));
            }
            // 4. Combine & Render
            ...
            if (!object._supportedEvents) {
                const events = typeof object.getEvents === 'function'
                    ? object.getEvents()
                    : ['onClick', 'onDragStart', 'onDragEnd', 'onDrop'];
                object._supportedEvents = events.map((evt) => ({ key: evt }));
            }
            this.setupBindings();
            this.runtime.setVariable('selectedObject', object);
            this.render();
     ...
     */
    renderInspectorDesigner(template) {
     ...
     */
    createGroupRow(group, _index, template) {
     ...
     */
    createPropertyRow(propConfig, _index, template) {
        const row = document.createElement('div');
        row.style.cssText = 'display:flex;align-items:center;gap:4px;padding:6px;background:#333;border-radius:4px';
        row.draggable = true;
        const handle = document.createElement('span');
     ...
     */
    openPropertyStyleEditor(propConfig, _template) {
        ...
        modal.querySelector('#style-save')?.addEventListener('click', () => {
            const colorInput = modal.querySelector('#style-color');
            const fontSizeInput = modal.querySelector('#style-fontSize');
            const bgColorInput = modal.querySelector('#style-bgColor');
            propConfig.style = {
     ...
     */
    exportInspectorLayout(template) {
     ...
     */
    importInspectorLayout(template) {
     ...
     */
    async loadDefaultLayout(template) {
        try {
            const response = await fetch('./editor/inspector_layout.json');
            if (!response.ok) {
            ...
            alert('Fehler beim Laden des Standard-Layouts: ' + error.message);
        }
    }
    /**
     * Loads a specific layout version from the layouts folder
     */
    async loadLayoutVersion(template, filename) {
        try {
    ...
    generateUIFromProperties(object, isMerging = false) {
        ...
        grouped.forEach((props, groupName) => {
            ...
            props.forEach((prop, idx) => {
                ...
                const labelStyle = { fontSize: 12, color: '#aaa' };
                if (layoutProp?.style) {
                    if (layoutProp.style.color)
                        labelStyle.color = layoutProp.style.color;
                    if (layoutProp.style.fontSize)
                        labelStyle.fontSize = parseInt(layoutProp.style.fontSize);
                    if (layoutProp.style.backgroundColor)
                        labelStyle.backgroundColor = layoutProp.style.backgroundColor;
                }
                // Label
                uiObjects.push({
                    className: 'TLabel',
     ...
     */
    expandForEach(objects, selectedObject) {
        const expanded = [];
        objects.forEach(obj => {
     ...
     */
    evaluateExpression(expr, selectedObject) {
        try {
    ...
    replaceTemplateVars(obj, item, index) {
        ...
        if (uiScope === 'library')
            ...
            uiEmoji = '🌎';
        else if (scope === 'local' || scope.startsWith('stage:'))
            uiEmoji = '🎭';
        else if (scope.startsWith('task:') || scope.startsWith('action:'))
            uiEmoji = '📍';
        else
            uiEmoji = '🎭'; // Default for other local scopes
        const replacements = {
            '${key}': item.key || index,
    ...
    render() {
        ...
        if (this.isFlowContext) {
            ...
            this.flowElements.forEach((el) => {
                ...
                if (el.data?.isLinked || el.data?.isEmbeddedInternal)
                    icon = '🔗' + icon;
    ...
    renderPropertiesTab(wrapper) {
        ...
        for (let i = 0; i < this.inspectorObjects.length; i++) {
            ...
            const isNextInput = nextObj && (nextObj.className === 'TEdit' ||
     ...
     */
    renderInlineRow(label, input) {
        // Visibility check - if either is hidden, hide the whole row
        if (!this.isObjectVisible(label) || !this.isObjectVisible(input))
            return null;
        ...
        const renderCurrentInput = () => {
            ...
            if (!isBlacklisted) {
                ...
                const updateToggleVisibility = (isHovering) => {
                    if (isBound) {
                ...
                toggle.onclick = (e) => {
                    ...
                    if (isBound) {
                        ...
                        else if (input.className === 'TNumberInput')
                            input.value = 0;
                        else if (input.className === 'TColorPicker')
                            input.color = '#ffffff';
    ...
    renderInput(obj) {
        ...
        if (className === 'TEdit') {
            ...
            if (obj.min !== undefined)
                input.min = obj.min.toString();
            if (obj.max !== undefined)
                input.max = obj.max.toString();
            if (obj.step !== undefined)
                input.step = obj.step.toString();
            input.style.flex = '1';
            input.style.padding = '4px 6px';
            input.style.backgroundColor = '#333';
            input.style.color = '#fff';
            ...
            if (obj.style) {
                ...
                if (obj.style.minWidth)
                    select.style.minWidth = obj.style.minWidth;
            }
            ...
            if (obj.source === 'tasks') {
                ...
                options = projectRegistry.getVariables().map(v => {
                    ...
                    if (v.uiScope === 'global')
                        emoji = '🌎';
                    else if (v.uiScope === 'local' || v.scope?.startsWith('task:') || v.scope?.startsWith('action:'))
                        emoji = '📍';
                    return { value: v.name, text: `${emoji} ${v.name}` };
                });
            }
            else if (obj.source === 'stages') {
                options = (this.project?.stages || []).map((s) => ({ value: s.id, text: s.name || s.id }));
            }
            else if (obj.source === 'services') {
                options = serviceRegistry.listServices().map(s => ({ value: s, text: s }));
            }
            else if (Array.isArray(obj.options)) {
                options = obj.options.map((opt) => typeof opt === 'string' ? { value: opt, text: opt } : { value: opt.value, text: opt.label || opt.text });
            }
            ...
            if (obj.style?.width)
                button.style.width = obj.style.width;
            button.onclick = () => {
                if (obj.action)
    ...
    renderObject(obj) {
        ...
        if (className === 'TLabel') {
            ...
            if (isGroupHeader) {
                ...
                if (obj.style?.flex)
                    el.style.flex = obj.style.flex;
                if (obj.style?.minWidth)
                    el.style.minWidth = obj.style.minWidth;
            }
        }
            ...
            if (obj.source === 'tasks') {
                ...
                if (Array.isArray(sourceArr)) {
                    options = sourceArr.map((opt) => typeof opt === 'string' ? { value: opt, text: opt } : { value: opt.value, text: (opt.label || opt.text || opt) });
                }
            }
            // Add empty option if allowed
            if (obj.allowEmpty) {
            ...
            let selectedVal = obj.selectedValue || obj.selected || '';
            let bindingExpression = null;
            if (selectedVal && String(selectedVal).startsWith('${')) {
                const expr = selectedVal;
                bindingExpression = expr;
            ...
            if (obj.style?.flex) {
                ...
                if (obj.style.minWidth)
                    button.style.minWidth = obj.style.minWidth;
            }
            else {
                    ...
                    meta.parameters.forEach(param => {
                        ...
                        if (param.type === 'object' || param.type === 'variable' || param.type === 'select' || param.type === 'stage') {
                            ...
                            if (param.source === 'objects')
                                items = projectRegistry.getObjects().map(o => o.name);
                            else if (param.source === 'variables')
                                items = projectRegistry.getVariables().map(v => v.name);
                            else if (param.source === 'stages')
                                items = (this.project?.stages || []).map((s) => s.id);
                            else if (param.source === 'services')
                                items = serviceRegistry.listServices();
            ...
            if (obj.style) {
                if (obj.style.display)
                    el.style.display = obj.style.display;
                if (obj.style.flexDirection)
                    el.style.flexDirection = obj.style.flexDirection;
                if (obj.style.alignItems)
                    el.style.alignItems = obj.style.alignItems;
                if (obj.style.gap)
                    el.style.gap = obj.style.gap;
                if (obj.style.marginBottom)
                    el.style.marginBottom = typeof obj.style.marginBottom === 'number' ? `${obj.style.marginBottom}px` : obj.style.marginBottom;
            }
            // Render children
            if (obj.children && Array.isArray(obj.children)) {
                obj.children.forEach((child) => {
     ...
     */
    validateInput(input, property, value, object) {
        if (!value)
            return;
    ...
    handleObjectChange(obj) {
        ...
        if (isNameChange) {
            if (isStageNameChange && this.project) {
                ...
                oldValue = selectedObject.Name || selectedObject.name;
            }
        }
        ...
        if (isNameChange && oldValue && value && oldValue !== value && this.project) {
            ...
            const isFlowElement = typeof selectedObject.getType === 'function';
            const flowType = isFlowElement ? selectedObject.getType() : null;
            ...
            if (flowType === 'Action') {
                ...
                this.project.stages?.some((s) => s.variables?.some((v) => v === selectedObject))) {
                RefactoringManager.renameVariable(this.project, oldValue, value);
            }
            else if (this.project.objects.some(o => o === selectedObject) ||
                this.project.stages?.some((s) => s.objects?.some((o) => o === selectedObject))) {
        ...
        if (selectedObject.data?.isLinked && isActionOrTask && !isParameter) {
            ...
            if (this.editor) {
                if (selectedObject.data.type === 'Action') {
                    ...
                    const index = targetCollection.findIndex((a) => a.name === name);
                    if (index !== -1) {
                    ...
                    const index = targetCollection.findIndex((t) => t.name === name);
                    if (index !== -1) {
                        targetCollection[index] = { ...targetCollection[index], ...selectedObject.data };
        ...
        if (previousValue !== value && !changeRecorder.isApplyingAction) {
            changeRecorder.record({
                type: 'property',
                description: `${selectedObject.name || 'Object'}.${propertyName} geändert`,
                objectType: 'object',
    ...
    getNestedProperty(obj, path) {
        ...
        for (const part of parts) {
            ...
            if (part === 'activeStage' && this.project) {
                current = this.project.stages?.find((s) => s.id === this.project?.activeStageId);
            }
            else {
     ...
     */
    async handleButtonAction(buttonDef) {
        ...
        switch (buttonDef.action) {
                ...
                fileInput.onchange = async (e) => {
                    ...
                    if (file) {
                        try {
                            ...
                            if (selectedObject && propName) {
                                selectedObject[propName] = base64;
                                // Also update backgroundImage if applicable
                                if (propName === 'src' && 'backgroundImage' in selectedObject) {
                                    selectedObject.backgroundImage = base64;
                                }
            ...
            case 'setTransparent': {
                ...
                if (selectedObject && propName) {
                    ...
                    if (propName.includes('.')) {
                        ...
                        selectedObject[propName] = 'transparent';
                    }
                    if (this.onObjectUpdate)
                        this.onObjectUpdate();
                    this.update(selectedObject);
            ...
            case 'openTaskEditor': {
                ...
                if (eventKey) {
                    ...
                    else if (selectedObject && selectedObject[eventKey]) {
                        // Variable Style: Direct property
                        taskName = selectedObject[eventKey] || '';
                    }
                }
            ...
            case 'openActionEditor': {
                ...
                const actionIndex = this.project?.actions.findIndex((a) => a.name === evaluatedActionName) ?? -1;
                ...
                if (result.action === 'save') {
                    ...
                    if (this.project) {
                        ...
                        const existingActionWithSameName = this.project.actions.find((a) => a.name === newName);
                        ...
                        if (existingActionWithSameName && !isSameAction) {
                            ...
                            const refInfo = refs.length > 0
                                ...
                                : '';
                            const choice = confirm(`Eine Action mit dem Namen "${newName}" existiert bereits.${refInfo}\n\n` +
                            ...
                            if (choice) {
                                ...
                                if (oldName && oldName.match(/^Aktion\d+$/)) {
                                    const oldIdx = this.project.actions.findIndex((a) => a.name === oldName);
                                    if (oldIdx !== -1) {
                                ...
                                if (selectedObject && typeof selectedObject.Name !== 'undefined') {
                                    ...
                                    if (selectedObject.data)
                                        selectedObject.data.name = newName;
                                }
                            ...
                            const finalActionIndex = this.project.actions.findIndex((a) => a.name === newName);
            ...
            case 'exportToLibrary': {
                ...
                const task = this.project?.tasks.find((t) => t.name === taskName);
                if (!task) {
                ...
                const activeStage = this.project?.stages?.find(s => s.id === this.project?.activeStageId);
                const flowChart = task.flowChart || task.flowGraph ||
            ...
            case 'createAction': {
                const newActionNameInput = document.querySelector('input[placeholder="New Action Name"]');
                const newActionName = newActionNameInput?.value.trim();
                if (!newActionName) {
                    alert('Please enter an action name');
                    return;
                }
                if (this.project?.actions.find((a) => a.name === newActionName)) {
                ...
                if (buttonDef.action === 'deleteVariable') {
                    ...
                    serviceRegistry.call('Dialog', 'showDialog', ['variable_editor', true, dialogData])
                        ...
                        if (result?.action === 'save' && result.data) {
                            ...
                            let newDefaultValue = data.defaultValue || '';
                            let newValue = data.value !== undefined ? data.value : newDefaultValue;
            ...
            case 'copyVariable': {
                ...
                while (this.project?.variables.find((v) => v.name === newName)) {
                    copyNum++;
