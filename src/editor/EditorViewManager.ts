...
import { KnowledgeBaseViewManager } from './knowledgebase/KnowledgeBaseViewManager';
...
export type ViewType = 'stage' | 'json' | 'run' | 'flow' | 'code' | 'management' | 'iframe' | 'userstories' | 'knowledgebase';
...
export class EditorViewManager {
    ...
    private knowledgeBaseViewManager!: KnowledgeBaseViewManager;
    ...
    constructor(private host: IViewHost) {
        ...
        this.knowledgeBaseViewManager = new KnowledgeBaseViewManager(this.host);
    ...
    public switchView(view: ViewType) {
        ...
        const knowledgebasePanel = document.getElementById('knowledgebase-viewer');
        ...
        if (knowledgebasePanel) knowledgebasePanel.style.display = 'none';
        if (iframePanel) {
        ...
        } else if (view === 'knowledgebase') {
            h.setRunMode(false);
            if (knowledgebasePanel) {
                knowledgebasePanel.style.display = 'block';
                knowledgebasePanel.style.height = '100%';
                knowledgebasePanel.style.overflowY = 'auto';
                this.knowledgeBaseViewManager.renderKnowledgeBaseView(knowledgebasePanel);
            }
        } else if (view === 'iframe') {
            h.setRunMode(false);
            if (stageWrapper) stageWrapper.style.display = 'none';
