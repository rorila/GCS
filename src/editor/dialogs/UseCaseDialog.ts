...
export class UseCaseDialog {
    ...
    private static generateAgentControllerScript(wData: any, stageId: string): string {
        const isTaskCall = wData.triggerType === 'taskCall';
        const isNone = wData.triggerType === 'none';
        ...
        const eventParamCode = wData.eventParam
            ...
            : '';
        const triggerCode = isTaskCall
            ? `// -- 4. Task-Aufruf verknüpfen -------------------\nagentController.addTaskCall('${wData.callerTask}', '${wData.taskName}');`
            : isNone
                ? `// -- 4. Kein Auslöser ------------------------------\n// Task '${wData.taskName}' wird nicht automatisch ausgelöst.`
                : `// -- 4. Event verknüpfen -----------------------\nagentController.connectEvent('${stageId}', '${wData.compName}', '${wData.eventName}', '${wData.taskName}');`;
        const triggerFields = isTaskCall
            ? `\n  trigger: { kind: 'taskCall', callerTask: '${wData.callerTask}' },`
            : isNone
                ? `\n  trigger: { kind: 'none' },`
...
agentController.addUseCase('${stageId}', {
  ...
  priority: '${wData.priority}',${triggerFields}
...
${wData.agentHints ? `\n// Hinweise: ${wData.agentHints}` : ''}`;
    ...
    public showAddUseCaseDialog(stageId: string, prefilled?: { className?: string, name?: string }) {
        ...
        const TRIGGERS = [
            ...
            { id: 'taskCall',   icon: '📣', label: 'Wird von einer\nanderen Aufgabe\naufgerufen', compType: '',   event: '' },
            { id: 'none',       icon: '⏸️', label: 'Noch offen /\nmanuell',            compType: '',               event: '' },
        ...
        // Bekannte Task-Namen fuer die Caller-Auswahl (existierende Stage-Tasks + geplante Tasks anderer UseCases)
        const allTaskNames: string[] = [];
        (project.stages || []).forEach((s: any) => (s.tasks || []).forEach((t: any) => { if (t?.name) allTaskNames.push(t.name); }));
        (project.userStories?.userStories || []).forEach((us: any) => { if (us?.plannedTask) allTaskNames.push(us.plannedTask); });
        const uniqueTaskNames = [...new Set(allTaskNames)];
        ...
        const renderStep = (): string => {
            ...
            if (wizardStep === 3) {
                ...
                // Task-Aufruf: kein Komponenten-/Event-Formular, nur Caller-Task + Task-Name
                if (wData.triggerType === 'taskCall') {
                    return `
                <div style="${sectionStyle}">
                    <div style="font-size:18px;font-weight:bold;color:#fff;margin-bottom:6px;">📣 Welche Aufgabe ruft diese auf?</div>
                    <div style="color:#9090c0;font-size:13px;margin-bottom:16px;">
                        Du hast gewählt: <strong style="color:#60a0ff;">${trigger ? `${trigger.icon} ${trigger.label.replace(/\n/g,' ')}` : ''}</strong>
                    </div>
                    <div style="margin-bottom:12px;">
                        <label style="${labelStyle}">Aufrufende Aufgabe (Task):</label>
                        <input id="w-caller-task" type="text" list="w-caller-task-list" placeholder="z.B. StartGame"
                            value="${wData.callerTask}" style="${inputStyle}">
                        <datalist id="w-caller-task-list">
                            ${uniqueTaskNames.map(n => `<option value="${n}">`).join('')}
                        </datalist>
                        <div style="color:#6060a0;font-size:11px;margin-top:4px;">💡 Tipp: Der Name einer existierenden oder geplanten Aufgabe</div>
                    </div>
                    <div>
                        <label style="${labelStyle}">Welchen Namen soll die aufgerufene Aufgabe (Task) haben?</label>
                        <input id="w-task" type="text" placeholder="z.B. SpawnBackupShooter" value="${wData.taskName}" style="${inputStyle}">
                        <div style="color:#6060a0;font-size:11px;margin-top:4px;">💡 Tipp: Kein Leerzeichen, fang mit Großbuchstaben an</div>
                    </div>
                </div>`;
                }

                // Kein Auslöser: nur Task-Name
                if (wData.triggerType === 'none') {
                    return `
                <div style="${sectionStyle}">
                    <div style="font-size:18px;font-weight:bold;color:#fff;margin-bottom:6px;">⏸️ Aufgabe ohne Auslöser</div>
                    <div style="color:#9090c0;font-size:13px;margin-bottom:16px;">
                        Diese Aufgabe wird (vorerst) nicht automatisch ausgelöst – z.B. eine Hilfs-Aufgabe.
                    </div>
                    <div>
                        <label style="${labelStyle}">Welchen Namen soll die Aufgabe (Task) haben?</label>
                        <input id="w-task" type="text" placeholder="z.B. HilfsBerechnung" value="${wData.taskName}" style="${inputStyle}">
                        <div style="color:#6060a0;font-size:11px;margin-top:4px;">💡 Tipp: Kein Leerzeichen, fang mit Großbuchstaben an</div>
                    </div>
                </div>`;
                }
            ...
            if (wizardStep === 6) {
                ...
                if (wData.triggerType === 'taskCall') {
                    diag += box('#6a1b9a', '📣 Aufruf durch Task', wData.callerTask||'?') + arrow;
                    diag += box('#2e7d32', 'Task', wData.taskName||'?');
                } else if (wData.triggerType === 'none') {
                    diag += box('#455a64', '⏸️ Auslöser', 'keiner / offen') + arrow;
                    diag += box('#2e7d32', 'Task', wData.taskName||'?');
                } else {
        ...
        const renderExpertMode = (): string => {
            ...
            <div style="${sectionStyle}">
                ...
                <div style="margin-bottom:10px;">
                    <label style="${labelStyle}">Auslöser-Art</label>
                    <select id="e-trigger-kind" style="${inputStyle}">
                        <option value="event" ${wData.triggerType!=='taskCall'&&wData.triggerType!=='none'?'selected':''}>⚡ Komponenten-Event</option>
                        <option value="taskCall" ${wData.triggerType==='taskCall'?'selected':''}>📣 Aufruf durch anderen Task</option>
                        <option value="none" ${wData.triggerType==='none'?'selected':''}>⏸️ Kein Auslöser (offen)</option>
                    </select>
                </div>
                <div id="e-event-fields" style="display:${wData.triggerType==='taskCall'||wData.triggerType==='none'?'none':'block'};">
                <div style="display:flex;gap:10px;margin-bottom:10px;">
                ...
                <div id="e-taskcall-fields" style="display:${wData.triggerType==='taskCall'?'block':'none'};margin-bottom:10px;">
                    <div style="display:flex;gap:10px;">
                        <div style="flex:1;"><label style="${labelStyle}">Aufrufender Task</label>
                            <input id="e-caller-task" type="text" list="e-caller-task-list" value="${wData.callerTask}" placeholder="z.B. StartGame" style="${inputStyle}">
                            <datalist id="e-caller-task-list">${uniqueTaskNames.map(n=>`<option value="${n}">`).join('')}</datalist></div>
                        <div style="flex:1;"><label style="${labelStyle}">Task-Name</label>
                            <input id="e-task2" type="text" value="${wData.taskName}" placeholder="z.B. SpawnBackupShooter" style="${inputStyle}"></div>
                    </div>
                </div>
                <div id="e-none-fields" style="display:${wData.triggerType==='none'?'block':'none'};margin-bottom:10px;">
                    <label style="${labelStyle}">Task-Name</label>
                    <input id="e-task3" type="text" value="${wData.taskName}" placeholder="z.B. HilfsBerechnung" style="${inputStyle}">
                </div>
                <div>
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
        ...
        const saveWizardData = () => {
            if (wizardMode === 'guided') {
                ...
                if (wizardStep === 3) {
                    ...
                    wData.callerTask = (document.getElementById('w-caller-task') as HTMLInputElement)?.value || wData.callerTask;
                ...
                wData.triggerType = (document.getElementById('e-trigger-kind') as HTMLSelectElement)?.value || 'event';
                ...
                wData.callerTask  = (document.getElementById('e-caller-task') as HTMLInputElement)?.value || wData.callerTask;
                wData.taskName    = (document.getElementById('e-task') as HTMLInputElement)?.value
                    || (document.getElementById('e-task2') as HTMLInputElement)?.value
                    || (document.getElementById('e-task3') as HTMLInputElement)?.value || '';
        ...
        const bindDialogListeners = () => {
            ...
            (window as any)._wSetTrigger = (id: string) => {
                ...
                if (id === 'taskCall' || id === 'none') { wData.compType=''; wData.compName=''; wData.eventName=''; wData.eventParam=''; }
                renderDialog();
            };
            ...
            // Experten-Modus: Auslöser-Art schaltet Felder um
            document.getElementById('e-trigger-kind')?.addEventListener('change', (e) => {
                const kind = (e.target as HTMLSelectElement).value;
                wData.triggerType = kind === 'event' ? (wData.triggerType === 'taskCall' || wData.triggerType === 'none' ? 'other' : wData.triggerType) : kind;
                const evFields = document.getElementById('e-event-fields');
                const tcFields = document.getElementById('e-taskcall-fields');
                const noFields = document.getElementById('e-none-fields');
                if (evFields) evFields.style.display = kind === 'event' ? 'block' : 'none';
                if (tcFields) tcFields.style.display = kind === 'taskCall' ? 'block' : 'none';
                if (noFields) noFields.style.display = kind === 'none' ? 'block' : 'none';
            });
            ...
            document.getElementById('w-save')?.addEventListener('click', () => {
                ...
                const trigger = wData.triggerType === 'taskCall'
                    ? { kind: 'taskCall' as const, callerTask: wData.callerTask }
                    : wData.triggerType === 'none'
                        ? { kind: 'none' as const }
                        : { kind: 'event' as const, component: { type: wData.compType, name: wData.compName }, event: wData.eventName, eventParam: wData.eventParam || undefined };
                project.userStories!.userStories!.push({
                    ...
                    interactions: [],
                    trigger,
                    plannedComponent: trigger.kind === 'event' ? trigger.component : undefined,
                    plannedEvent: trigger.kind === 'event' ? trigger.event : undefined,
                    plannedEventParam: trigger.kind === 'event' ? trigger.eventParam : undefined,
