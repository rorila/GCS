...
import { EditorViewManager } from './EditorViewManager';
...
export class Editor {
    constructor() {
        this.jsonInspector = null;
        this.jsonToolbox = null;
        this.flowEditor = null;
        this.flowToolbox = null;
        this.menuBar = null;
        ...
        this.runtime = null;
        this.useHorizontalToolbox = false;
        ...
        serviceRegistry.register('Editor', {
            ...
            jumpToDebug: (objectName, eventName) => {
                this.switchView('run');
                if (this.debugLog) {
        ...
        if (lastProject) {
            try {
                ...
                setTimeout(() => {
                    const toast = this.project.objects.find(o => o.className === 'TToast');
    ...
    get localPlayerNumber() { return this._localPlayerNumber; }
    // --- DELEGATIONS ---
    get currentObjects() { return this.stageManager.currentObjects(); }
    get currentActions() { return this.stageManager.currentActions(); }
    get currentTasks() { return this.stageManager.currentTasks(); }
    get currentVariables() { return this.stageManager.currentVariables(); }
    getActiveStage() { return this.stageManager.getActiveStage(); }
    migrateToStages() { this.stageManager.migrateToStages(); }
    // --- END DELEGATIONS ---
    set currentObjects(objs) {
        ...
        if (activeStage) {
            const localObjs = objs.filter(obj => {
                const isAlreadyLocal = (activeStage.objects || []).some(o => o.id === obj.id) ||
                    (activeStage.variables || []).some(v => v.id === obj.id);
                if (isAlreadyLocal)
                    return true;
                // If it's NEW (not in any stage), it belongs here.
                const existsElsewhere = (this.project.stages || []).some(s => s.id !== activeStage.id && ((s.objects || []).some(o => o.id === obj.id) ||
                    (s.variables || []).some(v => v.id === obj.id)));
            ...
            activeStage.variables = localObjs.filter(o => o.isVariable);
            return;
        }
        // Legacy-Fallback
        if (this.currentContext === 'splash') {
            ...
            this.project.variables = objs.filter(o => o.isVariable);
        }
    }
    /**
     * Gibt die passende Liste (Global vs Stage) für eine neue Action zurück
     */
    getTargetActionCollection(actionName, action) {
        const activeStage = this.getActiveStage();
        // 1. Explicit Scope Check
        if (action?.scope === 'global')
            return this.project.actions || (this.project.actions = []);
        if (action?.scope === 'stage' && activeStage)
            return activeStage.actions || (activeStage.actions = []);
        if (!activeStage)
            return this.project.actions || (this.project.actions = []);
     ...
     */
    getTargetTaskCollection(taskName, task) {
        const activeStage = this.getActiveStage();
        // 1. Explicit Scope Check
        if (task?.scope === 'global')
            return this.project.tasks || (this.project.tasks = []);
        if (task?.scope === 'stage' && activeStage)
            return activeStage.tasks || (activeStage.tasks = []);
        if (!activeStage)
            return this.project.tasks || (this.project.tasks = []);
    ...
    startMultiplayer() {
        ...
        this.initJSONLobby().then(() => {
            ...
            jsonLobby.show(stageContainer, gameName, (playerNumber, _seed) => {
                ...
                // Initialize syncers
                const gameLoop = this.project.objects.find(o => o.className === 'TGameLoop');
                ...
                network.on((msg) => {
                    if (msg.type === 'remote_input') {
                        ...
                        if (newDirection !== currentDirection) {
                            ...
                            const vars = { direction: newDirection };
    ...
    get currentView() { return this.viewManager.currentView; }
    get pascalEditorMode() { return this.viewManager.pascalEditorMode; }
    get useStageIsolatedView() { return this.viewManager.useStageIsolatedView; }
    set useStageIsolatedView(v) { this.viewManager.useStageIsolatedView = v; }
    get jsonMode() { return this.viewManager.jsonMode; }
    set jsonMode(v) { this.viewManager.jsonMode = v; }
    get workingProjectData() { return this.viewManager.workingProjectData; }
    set workingProjectData(v) { this.viewManager.workingProjectData = v; }
    get isProjectDirty() { return this.viewManager.isProjectDirty; }
    set isProjectDirty(v) { this.viewManager.isProjectDirty = v; }
    // ─────────────────────────────────────────────
    ...
    getResolvedInheritanceObjects() {
        ...
        while (curr) {
            ...
            seen.add(curr.id);
            chain.push(curr);
            const parentId = curr.inheritsFrom;
    ...
    updateStagesMenu() {
        if (!this.menuBar || !this.project.stages)
            return;
        const stageItems = [
            {
                id: 'new-stage',
    ...
    async renderFlowDiagram(container) {
    ...
    async saveProject() {
        ...
        const projName = this.project.stages?.find((s) => s.type === 'main')?.gameName || this.project.meta.name || 'New Game';
    ...
    async exportHTMLCompressed() {
        if (this.flowEditor)
            this.flowEditor.syncAllTasksFromFlow(this.project);
        const exporter = new GameExporter();
        await exporter.exportHTMLCompressed(this.project);
    }
    async exportJSONCompressed() {
        if (this.flowEditor)
            this.flowEditor.syncAllTasksFromFlow(this.project);
        const exporter = new GameExporter();
    ...
    loadProject(data) {
        ...
        if (data.meta)
            this.project.meta = data.meta;
        if (data.stage && data.stage.grid)
            this.project.stage.grid = data.stage.grid;
        ...
        if (data.stages && data.stages.length > 0) {
            const hydratedStages = data.stages.map((s) => {
                return {
                    ...s,
                    objects: hydrateObjects(s.objects || []),
                    variables: hydrateObjects(s.variables || [])
                };
            });
        ...
        this.project.variables = hydrateObjects(data.variables || []);
    ...
    init() {
        ...
        this.stage.onObjectCopy = (id, x, y) => {
            if (this.stage.runMode) {
                ...
                const grid = this.project.stage?.grid || this.project.grid;
                ...
                if (this.runtimeObjects) {
                    for (const target of this.runtimeObjects) {
                        ...
                        if (target.id === id || !target.droppable)
                            continue;
                        // Target Rect
                        const tRect = {
                        ...
                        // AABB Intersection
                        const intersects = !(dropRect.right < tRect.left ||
     ...
     */
    applyRecordedAction(action, direction) {
     ...
     */
    applyPropertyChange(action, direction) {
        if (!action.objectId || !action.property)
            return;
     ...
     */
    applyDragChange(action, direction) {
        if (!action.objectId)
            return;
    ...
    removeObjectSilent(id) {
        ...
        if (activeStage) {
            ...
            const varIdx = (activeStage.variables || []).findIndex(v => v.id === id);
            if (varIdx !== -1) {
                console.log(`[Editor] Removing variable: ${activeStage.variables[varIdx].name}`);
        ...
        const varIdx = this.project.variables.findIndex(v => v.id === id);
        if (varIdx !== -1) {
            this.project.variables.splice(varIdx, 1);
            return;
        }
    }
    createObjectInstance(type, name, x, y) {
        let newObj;
        switch (type) {
            case 'Button':
    ...
    addObject(type, x, y) {
        ...
        const dialogContainers = this.currentObjects.filter(o => {
            const cn = o.className || o.constructor?.name;
    ...
    selectObject(id) {
        ...
        if (id) {
            ...
            if (this.jsonInspector)
                this.jsonInspector.update(obj || null);
        }
        else {
    ...
    setRunMode(running) {
        ...
        if (running) {
            ...
            this.syncStageObjectsToProject();
            runtimeInstance = new GameRuntime(this.project, undefined, {
                ...
                onStageSwitch: (stageId) => {
                    ...
                    const targetStage = this.project.stages?.find((s) => s.id === stageId);
                    if (targetStage) {
                        ...
                        if (rt) {
                            ...
                            if (this.runtimeObjects) {
                                this.activeGameLoop = this.runtimeObjects.find((o) => o.className === 'TGameLoop') || null;
            ...
            if (this.stage) {
                this.stage.onEvent = (objectId, eventName) => {
            ...
            if (this.runtimeObjects) {
                this.activeTimers = this.runtimeObjects.filter(obj => obj.className === 'TTimer');
            ...
            if (this.runtimeObjects) {
                const numberLabels = this.runtimeObjects.filter(obj => obj.className === 'TNumberLabel');
            ...
            if (this.runtimeObjects) {
                this.activeGameServers = this.runtimeObjects.filter(obj => obj.className === 'TGameServer');
            ...
            if (this.activeGameLoop && typeof this.activeGameLoop.stop === 'function') {
                this.activeGameLoop.stop();
            }
            this.activeInputControllers.forEach(ic => {
                if (typeof ic.stop === 'function')
                    ic.stop();
            });
            this.activeTimers.forEach(timer => {
    ...
    handleGameServerEvent(id, eventName, data) {
        if (!this.runtime)
            return;
    ...
    startAnimationTicker() {
        ...
        const tick = () => {
            ...
            if (hasTweens || !this.firstRunRenderDone) {
                this.render();
                if (!hasTweens)
                    this.firstRunRenderDone = true;
            }
            // Continue loop
     ...
     */
    resolveObjectPreview(obj, context) {
        ...
        if (previewObj.children && Array.isArray(previewObj.children)) {
            ...
            previewObj.children = previewObj.children.map((child) => this.resolveObjectPreview(child, context));
        }
        return previewObj;
    }
    /**
    ...
    refreshPascalView() {
        ...
        try {
            ...
            if (this.pascalEditorMode) {
                ...
                const textarea = document.getElementById('pascal-editor-textarea');
    ...
    async initJSONInspector() {
        try {
            ...
            this.jsonInspector.onObjectUpdate = () => {
                ...
                const selObj = this.stage.selectedObject;
    ...
    refreshJSONView() {
        ...
        modeSelect.onchange = () => {
            this.jsonMode = modeSelect.value;
            this.refreshJSONView();
        };
        ...
        if (this.useStageIsolatedView && activeStage) {
            if (this.workingProjectData.stages) {
                const workingStage = this.workingProjectData.stages.find((s) => s.id === activeStage.id);
                if (workingStage) {
                    try {
                    ...
                    if (allRelevantNames.size > 0) {
                        ...
                        const localTaskNames = new Set(existingLocalTasks.map((t) => t.name));
                        const relevantGlobalTasks = this.project.tasks.filter(t => allRelevantNames.has(t.name) && !localTaskNames.has(t.name));
                    ...
                    const localActionNames = new Set(existingLocalActions.map((a) => a.name));
                    ...
                    if (usedActionNames.size > 0 && this.project.actions) {
                        const relevantGlobalActions = this.project.actions.filter(a => usedActionNames.has(a.name) && !localActionNames.has(a.name));
    ...
    renderJSONTree(data, container, applyBtn) {
        JSONTreeViewer.render(data, container, this.jsonMode === 'editor', (newData) => {
            ...
            if (this.useStageIsolatedView && activeStage && this.workingProjectData.stages) {
                const idx = this.workingProjectData.stages.findIndex((s) => s.id === activeStage.id);
                if (idx !== -1) {
    ...
    applyJSONChanges() {
        ...
        if (confirmed) {
            ...
            const toast = this.project.objects.find(o => o.className === 'TToast');
            if (toast && typeof toast.success === 'function') {
                toast.success('Projekt-Daten wurden erfolgreich aktualisiert.');
            }
        }
    }
    /**
    ...
    updateAvailableActions() {
        if (!this.jsonInspector || !this.project || !this.project.actions)
            return;
        const activeStage = this.getActiveStage();
     ...
     */
    syncFlowChartsWithActions() {
        if (!this.project?.flowCharts || !this.project?.actions)
            return;
        let syncCount = 0;
    ...
    handleRecordingAction(action) {
        switch (action) {
            case 'record-start':
                ...
                if (name)
                    changeRecorder.startRecording(name);
                break;
            case 'record-stop':
            ...
            case 'recording-export':
                const currentRec = playbackEngine.currentRecording;
                if (currentRec) {
            ...
            case 'recording-import':
                ...
                input.onchange = (e) => {
                    ...
                    if (file) {
                        ...
                        reader.onload = (re) => {
                            try {
                                const rec = JSON.parse(re.target?.result);
    ...
    syncStageObjectsToProject() {
        ...
        if (projectStage) {
            ...
            projectStage.variables = allObjs.filter(o => o.isVariable);
