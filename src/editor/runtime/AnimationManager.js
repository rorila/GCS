...
export class AnimationManager {
    ...
    cancelTween(target, property) {
        this.activeTweens = this.activeTweens.filter(t => !(t.target === target && t.property === property));
        // Safety: If no more position tweens exist, clear isAnimating
        if (property === 'x' || property === 'y') {
            const hasMore = this.activeTweens.some(t => t.target === target && (t.property === 'x' || t.property === 'y'));
    ...
    update() {
        ...
        for (const tween of completedTweens) {
            ...
            if (tween.property === 'x' || tween.property === 'y') {
                ...
                const hasMorePositionTweens = this.activeTweens.some(t => t.target === tween.target && (t.property === 'x' || t.property === 'y'));
