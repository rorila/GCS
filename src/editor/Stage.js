...
export class Stage {
    ...
    constructor(containerId, initialGrid) {
        // Callback for when an object is dropped
        this.onDropCallback = null;
        this.onSelectCallback = null;
        this.onObjectMove = null;
        this.onObjectCopy = null;
        this.onDragStart = null;
        // ... existing properties
        this.dragGhost = null;
        this.isCopyDrag = false;
        this.onObjectResize = null;
        this.onCopyCallback = null; // Returns cloned object
        this.onPasteCallback = null; // Returns new ID
        // Multi-selection state
        this.selectedIds = new Set();
        this.isDragging = false;
        this.isResizing = false;
        this.resizeDirection = ''; // n, s, e, w, nw, ne, sw, se
        this.dragStart = null;
        this.dragObjId = null;
        ...
        this.lastRenderedObjects = []; // Track objects for runtime lookup (e.g. draggable)
        this.currentDragPath = [];
        this.dragStartTime = 0;
        this.initialDragPositions = new Map();
        this._runMode = false;
        this.startAnimation = 'none';
        this.startAnimationDuration = 1000;
        this.startAnimationEasing = 'easeOut';
        this._selectedObject = null; // Currently selected object (primary)
        this.onEvent = null;
    ...
    bindEvents() {
        ...
        this.element.addEventListener('mousedown', (e) => {
            ...
            const objEl = target.closest('.game-object');
            if (this.runMode) {
                // Runtime Interaction
                if (objEl) {
                    ...
                    if (id) {
                        ...
                        if (obj && (obj.draggable || obj.draggableAtRuntime)) {
                            ...
                            if (obj.dragMode === 'copy') {
                                ...
                                // Create ghost
                                const originalEl = this.element.querySelector(`[data-id="${id}"]`);
                                if (originalEl) {
                                    this.dragGhost = originalEl.cloneNode(true);
            ...
            const hitObject = target.closest('.game-object');
            ...
            if (hitObject && !isClientPanel) {
                ...
                if (id) {
                    ...
                    this.selectedIds.forEach(selectedId => {
                        const selectedEl = this.element.querySelector(`[data-id="${selectedId}"]`);
                        if (selectedEl) {
                ...
                if (isClientPanel) {
                    this.pendingClientPanelId = objEl.getAttribute('data-id');
                }
                else {
                    this.pendingClientPanelId = null;
        ...
        window.addEventListener('mousemove', (e) => {
            if (this.runMode) {
                if (this.isDragging && this.dragObjId && this.dragStart) {
                    ...
                    if (this.isCopyDrag && this.dragGhost) {
                        ...
                        const el = this.element.querySelector(`[data-id="${this.dragObjId}"]`);
                        if (el) {
            ...
            if (!this.dragStart || !this.dragObjId)
                return;
            ...
            if (this.isRectSelecting && this.rectStart) {
                ...
                if (this.selectionRectEl) {
                    ...
                    gameObjects.forEach(objEl => {
                        ...
                        if (htmlEl.getAttribute('data-align') === 'CLIENT')
                            return;
                    ...
                    const pendingId = this.pendingClientPanelId;
                    if (pendingId) {
    ...
    renderObjects(objects) {
        ...
        objects.forEach(obj => {
            ...
            if (align === 'NONE' || align === 'CLIENT')
                return; // Skip CLIENT in first pass
        ...
        const renderedElements = Array.from(this.element.querySelectorAll('.game-object'));
        ...
        sortedObjects.forEach((obj) => {
            ...
            if (!objId)
                return;
            let el = this.element.querySelector(`[data-id="${objId}"]`);
            let isNew = false;
            if (!el) {
            ...
            if (obj.style) {
                ...
                if (obj.style.fontSize)
                    el.style.fontSize = typeof obj.style.fontSize === 'number' ? `${obj.style.fontSize}px` : obj.style.fontSize;
                if (obj.style.fontWeight)
                    el.style.fontWeight = obj.style.fontWeight;
                if (obj.style.borderRadius)
                    el.style.borderRadius = typeof obj.style.borderRadius === 'number' ? `${obj.style.borderRadius}px` : obj.style.borderRadius;
            ...
            if (obj.showGrid && !this.runMode) {
                ...
                if (this.runMode && !el.runModeTraceDone) {
                    el.lastLoggedSrc = null;
            ...
            if (className === 'TCheckbox') {
                ...
                if (textSpan) {
                    ...
                    if (obj.style?.fontFamily)
                        textSpan.style.fontFamily = obj.style.fontFamily;
                }
            }
                ...
                const input = el.querySelector('input');
                if (input) {
                    if (parseFloat(input.value) !== obj.value)
                        input.value = String(obj.value || 0);
                    if (obj.min !== undefined && obj.min !== -Infinity)
                        input.min = String(obj.min);
                    if (obj.max !== undefined && obj.max !== Infinity)
                        input.max = String(obj.max);
                    if (obj.step !== undefined)
                        input.step = String(obj.step);
                    // Styles
                    ...
                    if (obj.style?.fontFamily)
                        input.style.fontFamily = obj.style.fontFamily;
                }
            }
                ...
                if (isNew) {
                    ...
                    el.querySelector('.btn-single')?.addEventListener('click', (e) => {
                        e.stopPropagation();
                        if (this.onEvent)
                            this.onEvent(obj.id, 'onSingleClick');
                    });
                    el.querySelector('.btn-multi')?.addEventListener('click', (e) => {
                        e.stopPropagation();
                        if (this.onEvent)
                            this.onEvent(obj.id, 'onMultiClick');
                    });
                }
                const titleEl = el.querySelector('.card-title');
                if (titleEl && titleEl.innerText !== obj.gameName)
                    titleEl.innerText = obj.gameName;
            }
            else if (className === 'TButton') {
                if (el.innerText !== (obj.caption || obj.name))
                ...
                if (obj.style?.fontSize)
                    el.style.fontSize = typeof obj.style.fontSize === 'number' ? `${obj.style.fontSize}px` : obj.style.fontSize;
                if (obj.style?.fontFamily)
                    el.style.fontFamily = obj.style.fontFamily;
                ...
                if (fs)
                    el.style.fontSize = typeof fs === 'number' ? `${fs}px` : fs;
                ...
                if (align === 'center')
                    ...
                    el.style.justifyContent = 'flex-start'; // Default left
            }
                ...
                if (el.innerText !== (obj.title || obj.caption || obj.name))
                    el.innerText = obj.title || obj.caption || obj.name;
                ...
                if (this.runMode) {
                    ...
                    if (layoutConfig && layoutConfig.properties) {
                        ...
                        const sortedProps = Object.values(layoutConfig.properties)
                        ...
                        layoutConfig.groups?.sort((a, b) => a.order - b.order).forEach((group) => {
                            ...
                            if (props && props.length > 0) {
                                ...
                                props.forEach((prop) => {
                                    ...
                                    if (prop.style?.color)
                                        label.style.color = prop.style.color;
                                    else
                                        label.style.color = '#aaa';
                                    if (prop.style?.fontSize)
                                        label.style.fontSize = prop.style.fontSize;
                                    if (prop.style?.backgroundColor)
                                        label.style.backgroundColor = prop.style.backgroundColor;
                ...
                if (obj.children && Array.isArray(obj.children)) {
                    ...
                    obj.children.forEach((child) => {
                        let childEl = this.element.querySelector(`[data-id="${child.id}"]`);
                        if (!childEl) {
                        ...
                        if (child.style) {
                            ...
                            if (child.style.color)
                                childEl.style.color = child.style.color;
                        }
                        // Content based on type
                        ...
                        if (this.selectedIds.has(child.id)) {
                            ...
                            if (!childEl.querySelector('.resize-handle')) {
                                ...
                                handles.forEach(dir => {
                                    ...
                                    if (handleStyles[dir].top)
                                        handle.style.top = handleStyles[dir].top;
                                    if (handleStyles[dir].bottom)
                                        handle.style.bottom = handleStyles[dir].bottom;
                                    if (handleStyles[dir].left)
                                        handle.style.left = handleStyles[dir].left;
                                    if (handleStyles[dir].right)
                                        handle.style.right = handleStyles[dir].right;
                                    if (handleStyles[dir].transform)
                                        handle.style.transform = handleStyles[dir].transform;
    ...
    // ========================
    showContextMenu(clientX, clientY, objectId) {
        this.hideContextMenu();
     ...
     */
    finishPlacingSelection(gridX, gridY) {
        if (this.clipboardObjects.length === 0 || !this.onPasteCallback)
            return;
        const newIds = [];
