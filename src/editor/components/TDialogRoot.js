...
export class TDialogRoot extends TWindow {
    constructor(name, x = 100, y = 100, width = 400, height = 300) {
        ...
        this._title = 'Dialog';
        // Dialog behavior
        this.modal = true; // Block background?
        this.closable = true; // Show close button?
        this.draggableAtRuntime = true; // Draggable at runtime?
        this.centerOnShow = true; // Center when shown?
        // Events (Task names to execute)
        this.onShowTask = ''; // Task to run when dialog shows
        this.onCloseTask = ''; // Task to run when dialog closes
        this.onCancelTask = ''; // Task to run when cancelled
    ...
    setupDragging(handle) {
        handle.onmousedown = (e) => {
            if (!this._dialogElement || !this.draggableAtRuntime)
                return;
            ...
            document.onmousemove = (e) => {
                if (!this._isDragging || !this._dialogElement)
                    return;
