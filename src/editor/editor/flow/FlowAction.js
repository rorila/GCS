...
export class FlowAction extends FlowElement {
    ...
    getActionDefinition() {
        ...
        if (this.data?.isLinked && this.projectRef && this.Name) {
            ...
            if (proj.activeStageId && proj.stages) {
                ...
                if (stage?.actions) {
                    action = stage.actions.find((a) => a.name === this.Name);
                    if (action)
                        return action;
                }
            }
    ...
    set variable(v) { this.variableName = v; }
    get operation() {
        const action = this.getActionDefinition();
    ...
    set resultVariable(v) {
        const action = this.getActionDefinition();
        if (action)
            action.resultVariable = v;
    }
    get calcStepsJSON() {
        const action = this.getActionDefinition();
    ...
    getActionDetails(action) {
        ...
        if (this.data?.parentParams) {
            try {
                ...
                // Interpolate changes
                const changes = clonedAction.changes || clonedAction.propertyChanges;
                if (changes) {
        ...
        if (displayAction.type === 'property') {
            ...
            const changes = displayAction.changes || displayAction.propertyChanges || {};
