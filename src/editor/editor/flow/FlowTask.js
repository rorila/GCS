...
export class FlowTask extends FlowElement {
    ...
    getInspectorProperties() {
        ...
        if (taskDef?.params && Array.isArray(taskDef.params)) {
            ...
            taskDef.params.forEach((paramDef) => {
                ...
                else if (paramType === 'sprite' || paramType === 'object')
                    inspectorType = 'string'; // Object references as string
                props.push({
                    name: `param_${key}`,
                    type: inspectorType,
    ...
    get triggerMode() {
        ...
        if (this.data?.triggerMode)
            return this.data.triggerMode;
        const taskDef = this.getTaskDefinition();
