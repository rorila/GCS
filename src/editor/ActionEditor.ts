...
export class ActionEditor {
    ...
    private currentType: ActionType;
    private currentTarget: string;
    private currentChanges: Record<string, any>;

    // For variable type
    private currentVariableName: string;
    private currentSource: string;
    private currentSourceProperty: string;

    // For service type
    private currentService: string;
    private currentMethod: string;
    private currentServiceParams: Record<string, string>;
    private currentResultVariable: string;

    // For property type - pending form values (to auto-add on save)

    // For calculate type
    private currentCalcSteps: CalcStep[] = [];
    private currentCalcResultVariable: string = '';
    ...
    private render() {
        ...
        types.forEach(t => {
            ...
            if (t.value === this.currentType) opt.selected = true;
            typeSelect.appendChild(opt);
        });
        typeSelect.onchange = () => {
            this.currentType = typeSelect.value as ActionType;
            this.render();
        };
    ...
    private renderVariableUI(body: HTMLElement) {
        // Variable Name
        ...
        visibleVars.forEach(v => {
            ...
            opt.value = v;
            opt.innerText = v;
            if (v === this.currentVariableName) opt.selected = true;
        ...
        keywords.forEach(kw => {
            ...
            opt.value = kw;
            opt.innerText = `[${kw}]`;
            if (kw === this.currentSource) opt.selected = true;
        ...
        this.project.objects.forEach(o => {
            ...
            if (o.name === this.currentSource) opt.selected = true;
            sourceSelect.appendChild(opt);
        });
        ...
        readableProps.forEach(p => {
            ...
            opt.value = p;
            opt.innerText = p;
            if (p === this.currentSourceProperty) opt.selected = true;
    ...
    private updatePreview(preview?: HTMLElement) {
    ...
    private renderPropertyUI(body: HTMLElement) {
        ...
        keywords.forEach(kw => {
            ...
            opt.value = kw;
            opt.innerText = `[${kw}]`;
            if (kw === this.currentTarget) opt.selected = true;
    ...
    private renderCalculateUI(body: HTMLElement) {
        ...
        this.getVisibleVariables().forEach(varName => {
            ...
            opt.value = varName;
            opt.text = varName;
            if (varName === this.currentCalcResultVariable) opt.selected = true;
            resultSelect.appendChild(opt);
        });
    ...
    private createCalcStepRow(step: CalcStep, index: number, previewDiv: HTMLElement): HTMLElement {
        const row = document.createElement('div');
        row.style.display = 'flex';
        row.style.gap = '8px';
        ...
        if (index > 0) {
            ...
            ['+', '-', '*', '/', '%'].forEach(op => {
                ...
                opt.value = op;
                opt.text = op;
                if (op === step.operator) opt.selected = true;
                opSelect.appendChild(opt);
            });

            opSelect.onchange = () => {
                step.operator = opSelect.value as any;
                this.updateCalcPreview(previewDiv);
            };
        ...
        [{ value: 'variable', label: 'Variable' }, { value: 'constant', label: 'Konstante' }].forEach(t => {
            ...
            opt.value = t.value;
            opt.text = t.label;
            if (t.value === step.operandType) opt.selected = true;
            typeSelect.appendChild(opt);
        });

        typeSelect.onchange = () => {
            step.operandType = typeSelect.value as any;
        ...
        if (step.operandType === 'variable') {
            ...
            this.getVisibleVariables().forEach(varName => {
                ...
                if (v.name === step.variable) opt.selected = true;
                varSelect.appendChild(opt);
            });
    ...
    private createChangeItem(property: string, value: any): HTMLElement {
        const item = document.createElement('div');
        item.className = 'action-item';
        item.style.padding = '6px';
        item.style.marginBottom = '4px';
