...
export class TaskEditor {
    constructor(project, taskName, onSave) {
        this.viewMode = 'list';
    ...
    createSequenceItemElement(seqItem, index, isReadOnly = false) {
     ...
     */
    openControlStructureEditor(seqItem, _index) {
        ...
        const createVarSelect = (currentValue, placeholder) => {
        ...
        let elseSelect = null;
        // FOR loop inputs
        let iteratorInput = null;
        let fromInput = null;
        let toInput = null;
        let stepInput = null;
        // FOREACH inputs
        let sourceArrayInput = null;
        let itemVarInput = null;
        let indexVarInput = null;
    ...
    renderPascalCodeView(container) {
     ...
     */
    async renderTaskFlowDiagram(container) {
    ...
    getFlowConnections() {
        ...
        const flowChart = this.project.flowCharts?.[this.taskName];
        ...
        const mainTaskNode = elements.find((e) => e.type === 'Task' && !e.id.startsWith('imported-'));
        ...
        const outgoingConnections = connections.filter((c) => c.startTargetId === mainTaskNode.id);
        ...
        const connectedElements = outgoingConnections
            .map((c) => elements.find((e) => e.id === c.endTargetId))
            .filter((e) => e !== undefined);
        console.log('[TaskEditor] connectedElements:', connectedElements);
        connectedElements.forEach((e, i) => {
        ...
        connectedElements.sort((a, b) => a.y - b.y);
     ...
     */
    renderFlowConnectionsUI(container) {
        ...
        const allElements = [
            ...flowConnections.actions.map(a => ({ type: 'action', ...a })),
            ...flowConnections.tasks.map(t => ({ type: 'task', ...t }))
        ].sort((a, b) => a.y - b.y);
     ...
     */
    createFlowConnectionItem(type, name, order) {
        const item = document.createElement('div');
        item.style.cssText = `
            display: flex; 
            align-items: center; 
     ...
     */
    openFlowActionEditor(actionName) {
        const action = this.project.actions.find(a => a.name === actionName);
        if (action) {
     ...
     */
    openFlowSubTaskEditor(taskName) {
