    ...
    getAllReferencedTaskNames() {
        ...
        this.getTasks('all', false).forEach(task => {
            const scanSeq = (seq) => {
                ...
                seq.forEach(item => {
                    ...
                    if (item.thenTask)
                        referenced.add(item.thenTask);
                    if (item.elseTask)
                        referenced.add(item.elseTask);
                    if (item.body)
                        scanSeq(item.body);
                });
            };
        ...
        if (this.project.stages) {
            this.project.stages.forEach(s => {
                if (s.variables)
                    s.variables.forEach(v => allPotentialHolders.push(v));
            });
        }
            ...
            const scanSeq = (seq) => {
                ...
                seq.forEach(item => {
                    if (item.type === 'task' && item.name === name)
                        refs.push(`Task: ${task.name} -> Task-Aufruf`);
                    if (item.thenTask === name)
                        refs.push(`Task: ${task.name} -> Then-Task`);
                    if (item.elseTask === name)
                        refs.push(`Task: ${task.name} -> Else-Task`);
                    if (item.body)
                        scanSeq(item.body);
                });
            };
