...
export class StageRenderer {
    ...
    public static renderTable(el: HTMLElement, obj: any, onEvent?: (id: string, event: string, data?: any) => void): void {
        console.log(`[StageRenderer.renderTable] Rendering ${obj.name} (${obj.id})`);
        el.style.flexDirection = 'column';
        el.style.overflow = 'hidden';
        el.style.display = 'flex';
        el.style.fontSize = '12px';
        ...
        if (rawData && typeof rawData === 'object' && !Array.isArray(rawData)) {
            sourceObj = rawData;
            if (Array.isArray(sourceObj.data)) rawData = sourceObj.data;
            else if (Array.isArray(sourceObj.items)) rawData = sourceObj.items;
        }
        if (!Array.isArray(rawData)) rawData = [];

        let cols: any[] = [];
        if (Array.isArray(obj.columns) && obj.columns.length > 0) cols = obj.columns;
        else if (typeof obj.columns === 'string' && obj.columns.trim().startsWith('[')) {
            try { cols = JSON.parse(obj.columns); } catch (e) { }
        }
        if (cols.length === 0 && sourceObj && Array.isArray(sourceObj.columns) && sourceObj.columns.length > 0) cols = sourceObj.columns;

        if (cols.length === 0 && rawData.length > 0) {
            const firstItem = rawData[0];
            if (typeof firstItem === 'object' && firstItem !== null) {
                cols = Object.keys(firstItem).map(key => ({ field: key, label: key.charAt(0).toUpperCase() + key.slice(1) }));
            } else {
        ...
        if (titleBar) titleBar.innerHTML = `<span>${obj.name}</span> <span style="font-weight:normal; opacity:0.6; font-size:11px;">(${rawData.length} Items)</span>`;

        if (scrollArea) {
            scrollArea.innerHTML = '';
            if (obj.displayMode === 'cards') {
                this.renderCards(scrollArea, el, obj, cols, rawData, onEvent);
            } else {
    ...
    private static renderCards(scrollArea: HTMLElement, el: HTMLElement, obj: any, cols: any[], rawData: any[], onEvent?: any) {
    ...
    private static renderStandardTable(scrollArea: HTMLElement, el: HTMLElement, obj: any, cols: any[], rawData: any[], onEvent?: any) {
