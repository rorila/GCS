...
/**
 * Geplanter Auslöser eines Tasks.
 * - 'event':    Task wird durch ein Komponenten-Event ausgelöst (→ connectEvent)
 * - 'taskCall': Task wird von einem anderen Task aufgerufen (→ addTaskCall)
 * - 'none':     Aufrufer/Auslöser noch offen (reiner Hilfs-Task)
 */
export type PlannedTrigger =
    | { kind: 'event'; component: PlannedComponent; event: string; eventParam?: string }
    | { kind: 'taskCall'; callerTask: string }
    | { kind: 'none' };
...
export interface UserStory {
    ...
    trigger?: PlannedTrigger; // Auslöser des geplanten Tasks (Event / Task-Aufruf / offen)
    plannedComponent?: PlannedComponent; // Legacy: wird bei trigger.kind==='event' gespiegelt
    plannedEvent?: string;               // Legacy: wird bei trigger.kind==='event' gespiegelt
    plannedEventParam?: string;          // Legacy: wird bei trigger.kind==='event' gespiegelt
    plannedTask?: string;
