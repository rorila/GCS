...
export class PascalGenerator {
    static span(text, color, asHtml) {
        if (!asHtml)
            return text;
        return `<span style="color: ${color};">${text}</span>`;
    }
    static VARIABLE_EVENTS = [
     ...
     */
    static generateFullProgram(project, asHtml = true, activeStage) {
        const lines = [];
        // Program Header
        ...
        if (globalVars.length > 0) {
            ...
            globalVars.forEach(v => {
                ...
                if (v.threshold !== undefined)
                    metadata += ` Threshold=${v.threshold}`;
                if (v.triggerValue !== undefined)
                    metadata += ` TriggerValue='${v.triggerValue}'`;
        ...
        if (activeStage) {
            const relevantTaskNames = new Set();
            ...
            stageObjects.forEach((obj) => {
                ...
                Object.values(tasksMap).forEach((t) => {
                    if (typeof t === 'string' && t)
                        relevantTaskNames.add(t);
                });
            });
            // 4. Local tasks are ALWAYS relevant for the stage view
            if (activeStage.tasks) {
                activeStage.tasks.forEach((t) => relevantTaskNames.add(t.name));
            }
            // Filter logic:
        ...
        if (relevantVars.length > 0) {
            ...
            relevantVars.forEach(v => {
                PascalGenerator.VARIABLE_EVENTS.forEach(event => {
                    // Check if event property exists and has a value
                    const targetTask = v[event];
                    if (targetTask) {
                        varLines.push(`${this.span('PROCEDURE', '#c586c0', asHtml)} ${this.span(v.name, '#9cdcfe', asHtml)}.${this.span(event, '#dcdcaa', asHtml)}();`);
                        varLines.push(`${this.span('BEGIN', '#c586c0', asHtml)}`);
                        varLines.push(`  ${this.span(targetTask, '#dcdcaa', asHtml)}();`);
                        varLines.push(`${this.span('END', '#c586c0', asHtml)};`);
                        varLines.push('');
                    }
                });
            });
     ...
     */
    static generateProcedure(project, taskName, indent = 0, sequenceOverride, asHtml = true, activeStage) {
        const task = project.tasks.find(t => t.name === taskName) || (activeStage?.tasks?.find((t) => t.name === taskName));
        const sequence = sequenceOverride || task?.actionSequence || [];
        const lines = [];
        const space = ' '.repeat(indent);
        // Header with Usage Comments
        const users = (activeStage?.objects || project.objects).filter((obj) => {
            const tasksMap = obj.Tasks || {};
    ...
    static renderSequenceToPascal(project, sequence, lines, indent, asHtml, activeStage) {
        const space = ' '.repeat(indent);
        ...
        sequence.forEach(item => {
            if (item.type === 'action' || !item.type) {
                ...
                lines.push(`${space}${this.getActionPascalCode(project, item.name, asHtml, activeStage, item.data)}`);
            }
                ...
                if (cond) {
                    ...
                    let sysVarComment = '';
                    if (cond.variable === 'hitSide')
                        sysVarComment = ` ${this.span('{ System variable: side of collision/boundary }', '#6a9955', asHtml)}`;
                    if (cond.variable === 'direction')
                        sysVarComment = ` ${this.span('{ System variable: move direction (up/down) }', '#6a9955', asHtml)}`;
    ...
    static getActionPascalCode(project, actionName, asHtml, activeStage, embeddedData) {
        const action = embeddedData || project.actions.find(a => a.name === actionName) || (activeStage?.actions?.find((a) => a.name === actionName));
        if (!action)
            return `${this.span(actionName, '#dcdcaa', asHtml)}();`;
        let code = '';
     ...
     */
    static parse(project, code, targetStage) {
        ...
        if (programMatch) {
            if (!project.meta)
                project.meta = { name: '', version: '1.0.0', author: 'User' };
        ...
        const jsonTasks = [...(targetStage?.tasks || project.tasks || [])];
        const matchedJsonTasks = new Set();
        codeTasks.forEach((codeData, taskName) => {
            ...
            if (task) {
                ...
                const potentialOldTask = jsonTasks.find(t => !matchedJsonTasks.has(t.name) &&
            ...
            if (task) {
                // Sync Parameters
                if (codeData.params) {
                    const paramPairs = codeData.params.split(';').filter((p) => p.trim());
                    task.params = paramPairs.map((pair) => {
                ...
                if (JSON.stringify(oldSequence) !== JSON.stringify(newSequence)) {
                    ...
                    if (targetStage?.flowCharts?.[taskName])
                        delete targetStage.flowCharts[taskName];
                    if (project.flowCharts?.[taskName])
        ...
        if (stageToUse && stageToUse.objects) {
            foundHandlers.forEach(h => {
                const obj = stageToUse.objects.find((o) => o.name === h.objectName);
                if (obj) {
                    if (!obj.Tasks)
                        obj.Tasks = {};
        ...
        if (targetVariables.length > 0) {
            foundHandlers.forEach(h => {
                ...
                const variable = targetVariables.find((v) => v.name === h.objectName);
                if (variable) {
                    // Check if it's a known variable event
                    if (PascalGenerator.VARIABLE_EVENTS.includes(h.eventName)) {
                        variable[h.eventName] = h.targetTask;
                    }
                }
            });
        }
        ...
        if (!targetStage) {
            ...
            if (targetStage.tasks) {
                targetStage.tasks = targetStage.tasks.filter((t) => foundTasks.has(t.name));
            }
    ...
    static parseVariables(project, block, scope, foundSet, targetStage) {
        ...
        while ((match = varRegex.exec(block)) !== null) {
            ...
            let variable = (targetStage?.variables?.find((v) => v.name === name && (v.scope || 'global') === scope)) ||
    ...
    static parseBodyToSequence(project, taskName, body, actionCounter = { count: 0 }, oldSequence = [], targetStage) {
        ...
        let i = 0;
        const consumeUntil = (targetToken) => {
            let content = '';
            ...
            while (i < tokens.length) {
                ...
                if (upperToken === 'IF') {
                    ...
                    if (tokens[i]?.trim().toUpperCase() === 'ELSE') {
                        ...
                        if (tokens[i]?.trim().toUpperCase() === 'BEGIN') {
                            i++; // consume BEGIN
                            const elseBody = parseBlock(oldItem?.elseBody || []);
    ...
    static getLogicSignature(sequence) {
    ...
    static getPreferredCasing(project, propName, targetStage) {
    ...
    static parseSimpleStatement(project, taskName, statement, index, oldItem, targetStage) {
        // Assignment
        ...
        if (assignMatch) {
            ...
            if (target.includes('.')) {
                ...
                if (oldItem && oldItem.type === 'action') {
                    const candidate = (targetStage?.actions || project.actions || []).find((a) => a.name === oldItem.name);
                ...
                if (!action) {
                    action = (targetStage?.actions || project.actions || []).find((a) => a.type === 'property' && a.target.toLowerCase() === objName.toLowerCase() && (a.changes && Object.keys(a.changes).some(k => k.toLowerCase() === propName.toLowerCase())));
                }
        ...
        if (statement.endsWith('()')) {
            ...
            const isTask = project.tasks.some(t => t.name === callName) || (targetStage?.tasks?.some((t) => t.name === callName));
            return {
