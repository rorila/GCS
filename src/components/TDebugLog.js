...
export class TDebugLog {
    constructor() {
        this.service = DebugLogService.getInstance();
    ...
    createUI() {
        ...
        const detailsCb = this.element.querySelector('#show-details-cb');
        detailsCb.addEventListener('change', () => {
            this.showDetails = detailsCb.checked;
            this.saveFilters();
            this.renderLogs(this.service.getLogs());
        });
        const objSelect = this.element.querySelector('#obj-filter');
        const evtSelect = this.element.querySelector('#evt-filter');
        if (objSelect) {
        ...
        const pauseBtn = this.element.querySelector('#pause-logs');
        if (pauseBtn) {
     ...
     */
    setFilters(objectName, eventName) {
        this.objectFilter = objectName;
        this.eventFilter = eventName;
        // Alle Typen aktivieren für den Fokus
        this.typeFilters = new Set(['Event', 'Task', 'Action', 'Variable', 'Condition']);
        this.showDetails = true;
        this.filterContainer.querySelectorAll('input[type="checkbox"][data-type]').forEach((cb) => cb.checked = true);
        const detailsCb = this.element.querySelector('#show-details-cb');
        if (detailsCb)
            detailsCb.checked = true;
        this.updateObjectDropdown();
        this.updateEventDropdown();
        this.saveFilters();
    ...
    loadFilters() {
        ...
        if (saved) {
            try {
                ...
                if (filters.types)
                    this.typeFilters = new Set(filters.types);
                if (filters.showDetails !== undefined)
                    this.showDetails = filters.showDetails;
                if (filters.object !== undefined)
                    this.objectFilter = filters.object;
                if (filters.event !== undefined)
                    this.eventFilter = filters.event;
                // Sync UI Checkboxes
                this.filterContainer.querySelectorAll('input[type="checkbox"][data-type]').forEach((cb) => {
                ...
                const detailsCb = this.element.querySelector('#show-details-cb');
                if (detailsCb)
                    detailsCb.checked = this.showDetails;
                this.updateObjectDropdown();
                this.updateEventDropdown();
            }
            catch (e) {
    ...
    updateEventDropdown() {
        ...
        let projectEvents = [];
        if (this.project) {
            const obj = this.project.objects.find((o) => o.name === this.objectFilter);
            if (obj && obj.Tasks) {
    ...
    shouldShowRecursive(e, parentMatched) {
    ...
    isContextMatch(e) {
        if (this.objectFilter && e.objectName !== this.objectFilter)
            return false;
        if (this.eventFilter && e.eventName !== this.eventFilter)
            return false;
        return true;
    }
    isDeepMatch(e) {
        if (!this.objectFilter)
            return false;
        if (e.data && (e.data.target === this.objectFilter || e.data.source === this.objectFilter))
            return true;
        return false;
    }
    matchesTypeHierarchy(e) {
        ...
        if (e.type === 'Event' && !this.typeFilters.has('Event'))
            return false;
        // Task needs Task
        if (e.type === 'Task' && !taskEnabled)
            return false;
        // Action needs Task + Action
        if (e.type === 'Action' && (!taskEnabled || !actionEnabled))
            return false;
        // Variable needs Task + Action + Variable
        if (e.type === 'Variable' && (!taskEnabled || !actionEnabled || !variableEnabled))
            return false;
        // Condition needs Task + Condition
        if (e.type === 'Condition' && (!taskEnabled || !conditionEnabled))
            return false;
        return true;
    }
    renderEntry(entry, container, level, parentMatched) {
