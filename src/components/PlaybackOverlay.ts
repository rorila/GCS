import { DragPoint, RecordedAction } from '../services/ChangeRecorder';
...
export class PlaybackOverlay {
    ...
    private ghostCursor: HTMLElement | null = null;
    ...
    constructor(parent: HTMLElement) {
        ...
        playbackEngine.onStateChange = (state) => {
            if (state === 'stopped') this.clearPath();
            if (state === 'playing') this.show();
        };
    }
    ...
    public show(): void {
        if (this.ghostCursor) this.ghostCursor.style.display = 'block';
    }

    public hide(): void {
        if (this.ghostCursor) this.ghostCursor.style.display = 'none';
    }

    public clearPath(): void {
        if (!this.ctx || !this.pathCanvas) return;
        this.ctx.clearRect(0, 0, this.pathCanvas.width, this.pathCanvas.height);
    }

    private update(_currentTime: number): void {
    ...
    private animateDrag(path: DragPoint[]): void {
        if (!this.ghostCursor || !this.ctx) return;
    ...
    private drawPointOnCanvas(x: number, y: number): void {
        if (!this.ctx) return;
