...
export class GameRuntime {
    get contextVars() { return this.variableManager.contextVars; }
    get projectVariables() { return this.variableManager.projectVariables; }
    get stageVariables() { return this.variableManager.stageVariables; }
    constructor(project, objects, options = {}) {
        ...
        if (options.startStageId && hasStages) {
            ...
            activeStage = project.stages.find((s) => s.type === 'splash') ||
                project.stages.find((s) => s.id === project.activeStageId) ||
                project.stages[0];
        }
        if (objects) {
    ...
    start() {
        ...
        if (this.isSplashActive) {
            if (this.project.splashAutoHide) {
                const duration = this.stage?.duration || this.project.splashDuration || 3000;
        ...
        switch (action) {
            ...
            case 'start':
                if (varDef.type === 'timer')
                    this.startTimer(name, varDef, params[0] || varDef.duration || 10);
                break;
            case 'stop':
