...
export interface AIUserStorySummary {
    ...
    agentHints?: string;
    /** Auslöser des geplanten Tasks: 'event' (connectEvent), 'taskCall' (addTaskCall) oder 'none' */
    triggerKind?: 'event' | 'taskCall' | 'none';
    /** Bei triggerKind 'taskCall': Name des aufrufenden Tasks */
    triggerCallerTask?: string;
}
...
export class ProjectContextBuilder {
    ...
    private summarizeUserStory(userStory: UserStory): AIUserStorySummary {
        ...
        // Trigger: explizites Feld bevorzugen, sonst aus Legacy-Feldern ableiten
        const trig = (userStory as any).trigger;
        if (trig?.kind === 'taskCall') {
            summary.triggerKind = 'taskCall';
            if (trig.callerTask) summary.triggerCallerTask = trig.callerTask;
        } else if (trig?.kind === 'none') {
            summary.triggerKind = 'none';
        } else if (trig?.kind === 'event' || userStory.plannedComponent?.name || userStory.plannedEvent) {
