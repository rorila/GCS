    ...
    static fromUserStories(
        ...
        return {
            ...
            oneShotExample: example?.operations
                ? JSON.stringify(this.sanitizeOperations(example.operations), null, 2)
                : '[]',
        };
    }

    /**
     * Entfernt Binärdaten (Base64, Data-URLs) aus den Operations-Params.
     * Solche Werte sind für die KI wertlos, blähen aber die KB massiv auf.
     */
    private static sanitizeOperations(operations: any[]): any[] {
        const sanitizeValue = (value: any): any => {
            if (typeof value === 'string') {
                if (value.startsWith('data:')) return '<data-url>';
                // Lange Base64-artige Strings (z.B. eingebettete Bilder)
                if (value.length > 500 && /^[A-Za-z0-9+/=\s]+$/.test(value)) return '<base64-data>';
                return value;
            }
        ...
        return (operations || []).map(op => ({ ...op, params: sanitizeValue(op?.params) }));
    }
