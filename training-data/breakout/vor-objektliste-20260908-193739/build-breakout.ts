...
for(const name of bricks) prop('NeuesSpiel',`Act_Reset_${name}`,{[`${name}.visible`]:true,[`${name}.collisionEnabled`]:true});
...
for(const [i,name] of bricks.entries()) {
    const t=`Treffer_${name}`;task(t,`Entfernt ${name}, gibt ${(5-Math.floor(i/8))*10} Punkte und prüft den Sieg.`);
    const once=`Werten_${name}`;task(once,'Wertet einen noch sichtbaren Stein genau einmal.');
    branch(t,`${name}.visible`,'==',true,once);
    prop(once,`Act_Entferne_${name}`,{[`${name}.visible`]:false,[`${name}.collisionEnabled`]:false});
    agent.addAction(once,'increment',`Act_Punkte_${name}`,{changes:{'GameState.score':(5-Math.floor(i/8))*10,Reststeine:-1}});
    prop(once,"Act_Anzeige_"+name,{'Punkte.text':'PUNKTE  ${GameState.score}'});
    branch(once,'Reststeine','==',0,'Sieg');event(name,'onCollision',t);
}
...
learn('punkte','Ergänze Werten_Stein_1_1 um Act_Entferne_Stein_1_1: Stein_1_1 unsichtbar machen und Kollision deaktivieren.',`agent.addAction('Werten_Stein_1_1','property','Act_Entferne_Stein_1_1',{changes:{'Stein_1_1.visible':false,'Stein_1_1.collisionEnabled':false}});`);
learn('punkte','Ergänze Werten_Stein_1_1 um Act_Punkte_Stein_1_1: 50 Punkte auf GameState.score addieren und Reststeine um eins senken.',`agent.addAction('Werten_Stein_1_1','increment','Act_Punkte_Stein_1_1',{changes:{'GameState.score':50,Reststeine:-1}});`);
learn('punkte','Rufe im Task Werten_Stein_1_1 den Task Sieg auf, wenn Reststeine null ist.',`agent.addBranch('Werten_Stein_1_1','Reststeine','==',0,b=>b.addTaskCall('Sieg'));`);
