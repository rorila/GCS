...
## Unreleased (WIP)
- **Action Parameter Formatierung (Phase 19):** Verbesserte Lesbarkeit von Zuweisungsparametern (`type: "property"`) im Inspector. Anstelle von generischen Objekten (`[object Object]`) werden Zuweisungen jetzt als klartext lesbare Strings gerendert (z.B. `text := 'anmelden'`). Änderungen im Eingabefeld werden beim Speichern (OnChange) bidirektional in korrekte JSON-Objekte zurückgewandelt.
