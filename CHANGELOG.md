...
## [3.32.0] - 2026-05-12
- **Feature (Variable Max Intervalle)**: Die Komponente `TTimer` unterstützt nun Variablen und Ausdrücke im Feld **Max Intervalle** (z. B. `${myLimit}`). Dies ermöglicht die dynamische Konfiguration der Timer-Laufzeit basierend auf dem Spielzustand. Zur Unterstützung wurde das `IRuntimeComponent.initRuntime`-Interface um den Zugriff auf `contextVars` erweitert.
- **Feature (Variable Pool Size)**: Für die Komponente `TSpriteTemplate` kann nun im Feld **Pool Size** statt einer festen Zahl auch eine Variable oder ein Ausdruck (z. B. `${myPoolSize}` oder `${stageLimit + 10}`) eingegeben werden. Der Inspector wurde hierfür von einem rein numerischen Feld auf ein Textfeld umgestellt. Die Auswertung erfolgt automatisch zum Zeitpunkt der Pool-Initialisierung durch die GameRuntime. Dies ermöglicht hochdynamische Object-Pooling-Szenarien, die sich an globale Einstellungen oder Level-Parameter anpassen.

### 2026-05-09
- **Bugfix (Action Creation via Drag & Drop)**: Beim Erstellen einer neuen Action per Drag & Drop aus der Palette in den Flow-Editor blieben die Action-Eigenschaften zunächst leer, da die Defaults in `FlowNodeFactory.ts` noch nicht über den `SchemaMigrator` aufgefüllt wurden. Dies wurde korrigiert, sodass Actions nun direkt beim Hineinziehen (z.B. vom Typ `animate`) mit ihren korrekten Default-Parametern initialisiert werden.
- **SYNC_REFACTOR Phase 3.5 (Cleanup)**: Finalisierung des Sync-Refactorings. Es wurde ein Fehler behoben, bei dem neue Actions oder Typ-Wechsel im Inspector leere Felder (ohne Default-Werte) anzeigten, da diese bisher nur beim initialen Laden (`applyRegistryDefaults`) injiziert wurden. Die Methode `SchemaMigrator.initializeActionDefaults()` sorgt nun in `AgentController` und `FlowAction.applyChange` dafür, dass Default-Werte sofort auf der SSoT und im Proxy aufgefüllt werden. Zudem wurde in `SyncValidator.validate` das automatische Repairing standardmäßig deaktiviert, um Daten-Drifts nicht mehr stumm zu maskieren (explizit auf `true` in `FlowSyncManager`). Das Feld `schemaVersion` wurde für Typsicherheit ins `GameProject`-Interface aufgenommen.
- **SYNC_REFACTOR Phase 3 (applyChange-only Schreibpfad)**: Die Architektur für die Datenmutation im Editor wurde grundlegend umgestellt. Statt des fehleranfälligen Doppel-Dispatches (Inspector-Change -> SSoT -> applyChange -> SSoT) läuft nun sämtliche Mutation von FlowNodes ausschließlich über `applyChange()`. Der Bulk-Sync von Properties in `FlowSyncManager.syncToProject()` wurde entfernt, sodass `syncToProject` nun nur noch für Topologie (Nachregistrierung fehlender Actions) zuständig ist. Die `InspectorSectionRenderer`-Handler für Select, Number und Checkbox wurden refaktorisiert und dedupliziert. 40 Tests der Regression-Suite bestanden.
- **SYNC_REFACTOR Phase 2 (wasMissing entfernt)**: Die 3 `wasMissing`-Blöcke im `InspectorSectionRenderer`, die zur Render-Zeit Default-Werte in die SSoT schrieben (Side-Effect im Render-Pfad), wurden entfernt. Stattdessen füllt `SchemaMigrator.applyRegistryDefaults()` beim Laden fehlende Defaults aus den `ActionRegistry`-Metadaten auf. Render ist ab sofort read-only. 4 neue Tests, alle grün.
- **SYNC_REFACTOR Phase 1 (Schema-Normalisierung)**: Neuer `SchemaMigrator` migriert beim Laden automatisch 5 Legacy-Alias-Felder auf kanonische Namen (`actionType`→`type`, `propertyChanges`→`changes`, `variable`→`variableName`, `methodName`→`method`, `expression`→`formula`). Schema-Version wird auf `4.0.0` gesetzt. Hook in Editor (`EditorDataManager.loadProject`) und Standalone-Player (`player.ts`). 8 neue Tests, alle grün.
- **SYNC_REFACTOR Phase 0 (Test-Netz)**: 4 neue Test-Suiten mit insgesamt 28 Tests erstellt, die die Inspector↔JSON-Synchronisation deterministisch absichern: Layer A (Store SET_PROPERTY), Layer B (SyncValidator R1-R6 Strict), Layer C (FlowAction-Alias Roundtrip, Inspector Writeback/applyChange). Alle Tests grün.
