...
test('DEBUG: chicken piece inspection', async ({ page }) => {
    ...
    await page.locator('#run-stage .image-gallery-tile[data-path$="cartoon-chicken-7356485_640.png"]').click();
    ...
    const info = await page.evaluate(() => {
        ...
        const pieces = rt.objects.filter((o: any) => o.isPoolInstance);
        return {
            ...
            pieces: pieces.map((p: any) => ({ name: p.name, match: p.matchValue, bg: p.backgroundImage, mode: p.appearanceMode, sx: p.sourceRectX, sy: p.sourceRectY, sw: p.sourceRectWidth, sh: p.sourceRectHeight, srcW: p.sourceWidth, srcH: p.sourceHeight, w: p.width, h: p.height }))
        };
    });
