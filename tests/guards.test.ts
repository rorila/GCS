...
export async function runGuardTests(): Promise<TestResult[]> {
    ...
    try {
        ...
        const ok = oversized.length <= 12; // Baseline für bestehende Legacy-Dateien (inkl. AgentController & UserStoriesViewManager)
        addResult('Guard: Dateigroesse < 1000 Zeilen', ok, ok ? `Baseline 12 eingehalten, Aktuell: ${oversized.length}` : `Zu gross: ${oversized.join(', ')}`);
    } catch (e: any) {
