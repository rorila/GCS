...
export async function runGuardTests(): Promise<TestResult[]> {
    ...
    try {
        ...
        const ok = oversized.length <= 10; // Baseline für bestehende Legacy-Dateien
        addResult('Guard: Dateigroesse < 1000 Zeilen', ok, ok ? `Baseline 10 eingehalten, Aktuell: ${oversized.length}` : `Zu gross: ${oversized.join(', ')}`);
    } catch (e: any) {
