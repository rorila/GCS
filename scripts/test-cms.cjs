...
await event('Anmelden');check('Einwahl öffnet Räume',(await state()).Modus.value==='rooms');check('Zwei Räume sichtbar',(await state()).Karte1.visible===true);
await event('Karte0');check('Raum öffnet Galerie',(await state()).Modus.value==='games');await page.screenshot({path:path.join(root,'docs/cms-galerie.png')});await event('Karte0');await page.waitForFunction(()=>document.querySelector('iframe')?.src.includes('/play/'));check('Spiel im Host geöffnet',true);
