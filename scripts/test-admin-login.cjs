...
(async()=>{
 ...
 try{
  ...
  const clientFile=path.join(__dirname,'../game-server/public/projects/GCS-CMS-Anmeldung.json'),client=JSON.parse(fs.readFileSync(clientFile));client.stages.find(s=>s.type==='main').objects.find(o=>o.name==='Titel').text='Meine Verwaltung';fs.writeFileSync(temporary,JSON.stringify(client));check('Dialog stammt aus Projektdatei',renderLogin(temporary).includes('Meine Verwaltung')&&!renderLogin(temporary).includes('<form'));
  ...
  await page.getByText('Verwaltung öffnen →',{exact:true}).click();await page.waitForFunction(()=>window.player?.runtime?.getObjects().some(o=>o.name==='Raeume'));check('Verwaltung im selben Fenster geöffnet',true);
