/**
 * Headless-Simulation: SuperMario_Templates.json — Level wird zur Laufzeit
 * aus TSpriteTemplates (SpritePool) gespawnt. Prueft Pool-Vererbung
 * (gravity/pushOutOnCollision/parentId), Landung, Kamera, Muenze,
 * Goomba-Stomp und Gruben-Tod.
 * Ausfuehren: npx tsx scripts/test_mario_templates.ts
 */
...
const project = JSON.parse(readFileSync(join(__dirname, '../game-server/public/projects/SuperMario_Templates.json'), 'utf8'));
const bp = project.stages[0], main = project.stages[1];

// ─── Objekte instanziieren (Welt-Kinder rekursiv, wie flattenWithChildren) ───
const objects: any[] = [];
const panels: any[] = [];
...
// ─── SpritePools initialisieren (wie RuntimeObjectService.initSpritePools) ───
...
// ─── spawnObject/destroyObject (wie RuntimeObjectService) ───
const context: any = {
...
async function runSeq(seq: any[], vars: Record<string, any>, depth = 0): Promise<void> {
    for (const item of seq) {
        if (item.type === 'action') await runAction(actionsByName.get(item.name), vars);
            ...
            // Inline-Action (z.B. spawn_object/destroy_object direkt im Body)
...
console.log('=== SpielInit via Welt.onStart (vor erstem Tastendruck) ===');
await fireEvent('Welt', 'onStart');
check('Level gebaut: 127 Instanzen aktiv', pool.getActiveInstances().length === 127, `=${pool.getActiveInstances().length}`);

console.log('=== Ready-Bildschirm: Mario darf nicht fallen ===');
simFrames(180); // 3s warten
check('Mario steht noch (y=17)', Math.abs(mario.y - 17) < 0.01, `y=${mario.y.toFixed(3)}`);
...
check('Gravitation an', mario.gravity === 1.5, `=${mario.gravity}`);

console.log('=== Pool-Vererbung ===');
const aktiv = pool.getActiveInstances();
check('127 Instanzen aktiv', aktiv.length === 127, `=${aktiv.length}`);
const bodenTile = objects.find(o => o.name === 'TplBodenG_pool_0');
const goomba0 = objects.find(o => o.name === 'TplGoomba_pool_0');
check('Instanz erbt parentId=Welt', bodenTile?.parentId === 'stage_main_Welt', `=${bodenTile?.parentId}`);
check('Tile erbt pushOutOnCollision', bodenTile?.pushOutOnCollision === true, `=${bodenTile?.pushOutOnCollision}`);
check('Goomba erbt gravity', goomba0?.gravity === 1.5, `=${goomba0?.gravity}`);
check('Goomba erbt velocityX', goomba0?.velocityX === -0.035, `=${goomba0?.velocityX}`);
check('Instanz erbt backgroundImage', !!bodenTile?.backgroundImage, '');
const roehre2 = objects.find(o => o.name === 'TplRoehre_pool_2');
check('Groesse nachgezogen (Roehre h=4)', roehre2?.height === 4, `h=${roehre2?.height}`);
...
const goombas = objects.filter(o => o.name?.startsWith('TplGoomba_pool'));
...
console.log('=== Muenze (destroy_object -> Pool-Release) ===');
const muenze = objects.find(o => o.name === 'TplMuenze_pool_0'); // (8,16)
const muenzenVorher = Number(contextVars.Muenzen);
const aktivVorher = pool.getActiveInstances().length;
mario.x = 8; mario.y = 16; mario.velocityX = 0; mario.velocityY = 0;
simFrames(1);
await new Promise(r => setTimeout(r, 0));
check('Muenze eingesammelt (released)', !muenze.visible, `visible=${muenze.visible}`);
...
check('Instanz zurueck im Pool', pool.getActiveInstances().length === aktivVorher - 1, `aktiv=${pool.getActiveInstances().length}`);

console.log('=== Goomba-Stomp ===');
const goomba = objects.find(o => o.name === 'TplGoomba_pool_0');
goomba.visible = true;
goomba.x = 30; goomba.y = 18; goomba.velocityX = 0; goomba.velocityY = 0;
mario.x = 30; mario.y = 16.5; mario.velocityX = 0; mario.velocityY = 0.2;
cm.clearTrackingFor(mario.id); cm.clearTrackingFor(goomba.id);
simFrames(3);
await new Promise(r => setTimeout(r, 0));
check('Goomba released (visible=false)', !goomba.visible, `visible=${goomba.visible}`);
check('Stomp-Bounce (vy<0)', mario.velocityY < 0, `vy=${mario.velocityY}`);

console.log('=== Gruben-Tod ===');
contextVars.Unbesiegbar = 0;
const lebenVorher = Number(contextVars.Leben);
mario.x = 31.5; mario.y = 25; mario.velocityY = 0.3;
simFrames(1);
await new Promise(r => setTimeout(r, 0));
