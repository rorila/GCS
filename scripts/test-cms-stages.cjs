...
check('Sechsundzwanzig Oberflächen und zwölf Server-Stages',p.stages.filter(s=>!s.id.startsWith('stage_server_')).length===26&&p.stages.filter(s=>s.id.startsWith('stage_server_')).length===12);
...
const password=crypto.randomBytes(18).toString('hex'),salt=crypto.randomBytes(16).toString('hex');fs.writeFileSync(path.join(dir,'cms-admin-auth.json'),JSON.stringify([{personId:'demo-adult',username:'navadmin',salt,hash:crypto.scryptSync(password,salt,64).toString('hex')}]));
...
// Bereichsnavigation ist berechtigungsabhängig — SuperAdmin sieht alle Ziele.
await page.locator('[name=username]').fill('navadmin');await page.locator('[name=password]').fill(password);await page.locator('[name=password]').press('Enter');
await page.waitForFunction(()=>window.player.runtime.stage.id==='stage_super');await page.waitForTimeout(700);
// Super-Sidebar fuehrt nur zu Super-Bereichen. Die Anmeldeseite hat keine
// Bereichsnavigation mehr (Sitzungsschutz); mit Sitzung: „Weiter" / „Abmelden".
for(const id of ['stage_super_houses','stage_super_admins','stage_library','stage_super','stage_admin_login'])await navigate(id);
await page.waitForFunction(()=>window.player.runtime.getObjects().find(o=>o.name==='SitzungWeiter')?.visible===true);
check('Anmeldeseite zeigt bestehende Sitzung statt Bereichsnavigation',await page.evaluate(()=>!window.player.runtime.getObjects().some(o=>o.name==='Navigation_stage_house')));
...
await page.locator('[data-id="'+await page.evaluate(()=>window.player.runtime.getObjects().find(o=>o.name==='SpielerEinwahl').id)+'"]').click();await page.waitForFunction(()=>window.player.runtime.stage.id==='stage_main');await page.waitForTimeout(450);check('Rückkehr zur Emoji-Stage',await page.evaluate(()=>window.player.runtime.getObjects().some(o=>o.name==='Emoji_dog')));check('Keine Browserfehler',errors.length===0);
